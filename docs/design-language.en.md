...
When the official account launcher is present at the bottom of the sidebar, Settings and Remote pairing open from its menu; neither a second Settings button nor a separate Remote row appears. Without an account launcher, the original Settings button and Remote row remain. Update and connection status remain independent bottom entries, and an empty status row takes no space. Both Settings paths open the same panel. Closing Settings or the Remote popup opened from the account menu returns focus to the account button. The Remote menu item uses the existing 16px phone outline and Menu primitive, and opens the existing pairing popup. Desktop deep links retain an invisible Settings trigger so the pet and keyboard shortcuts can open a section directly without adding a visible entry.
...
The 48px window caption drag band covers only space outside controls. Conversation hierarchy navigation, Agent actions, the directory opener, and plugin seats at either end of the Session header must subtract clickable hit areas from that band on Windows and macOS. The current Session title is a read-only breadcrumb that cannot navigate to itself, and the Agent preset name is a read-only status label; ancestor Session titles, Agent Team, and other actual buttons must respond to clicks.
