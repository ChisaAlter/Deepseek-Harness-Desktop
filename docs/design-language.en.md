...
16. **The sidebar brand is Whale Isle.** `setup:harness` runs the vendor tree's own `pnpm run build:official` (`DSH_CLIENT_BUILD_PROFILE=official`). Desktop and Web official builds use the same sidebar slots for the transparent whale-girl head, same-line “鲸屿 / WHALE ISLE”, and small “BASED ON DEEPSEEK HARNESS” beneath them. The 屿 glyph uses brand blue. Neither shows a square badge or the local-build fallback; the collapsed rail shows the panel icon. The wordmark may use compact type to fit the 40px Windows title bar without changing control typography. Rebuild client changes with that same command; a lone `build:lib:client` bakes the local-build brand back in.
