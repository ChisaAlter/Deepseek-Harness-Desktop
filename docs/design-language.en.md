...
The scene page has two zones: top status and centered startup content, sharing the available window height without overlapping or being wrapped in cards. Keep `DSH-DESKTOP`, the state stamp, and the four L-shaped corner rails at the top; place the metadata below the title bar and native window controls. The stamp follows `body[data-state]`: idle / IDLE, starting / BOOT, ready / READY, stopping / HALT, or error / ERROR. The center uses the existing whale loading animation [`assets/whale-spin.svg`](../assets/whale-spin.svg), unmodified at 112px with its 2-second loop and `.mark` above the scan overlay; `prefers-reduced-motion` swaps it for the static [`assets/whale-head.png`](../assets/whale-head.png). Then show `Whale Isle`, current status, and its hint. Type hierarchy is brand 16/24, status 14/22, and secondary detail 12/18.

Logs and diagnostics live on a details page (two-page structure): the scene's bottom edge carries one monospace handle, "详细 · 日志 NN", counting log lines live. The details page is a full-page overlay on the same instrument canvas — a small whale mark plus a `BOOT LOG` column header over a hairline, then left-aligned failure details, the recovery actions (retry / cancel auto-restart / launcher bridge / download log), and the monospace log column inside a scrolling column; "回到场景" or Escape returns. The details page opens automatically whenever the action surface appears (`error`, recovery `scheduled` / `restarting`); once the user closes it manually it stays closed until the action surface clears. Logs stay unboxed monospace 14/22 and clear the corner rails (`--boot-log-inset`). Responsive rows fit the main window's 960×640 minimum; shorter windows reduce empty space while the details column still scrolls to the full error and actions. Show only progress reported by controller or plugin events; do not estimate percentages or invent steps. Plugin loading stays on the boot page, and the official Web UI appears only after its background BrowserView is ready. Show the launcher bridge only for a settled `error` whose recovery status is neither `scheduled` nor `restarting`.
