...
| Desktop boot page | Mark / copy `rise` (8px + fade, staggered 0 / 80 / 120 / 160ms); stamp `pulse` 1.2s; whale SVG `spin` 2s infinite loading loop; log lines `fade`. Entrance/exit durations use baseline tokens; reduced motion stops all of them and replaces the loader with the static head | [`boot.css`](../src/renderer/boot.css). The instrument look must not spread; see [Desktop boot page](design-language.en.md#desktop-boot-page) |
