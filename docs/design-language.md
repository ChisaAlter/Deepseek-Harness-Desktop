...
侧栏底部有官方账户入口时，设置与远程配对入口都收进该账户菜单，不再并列显示第二个设置按钮或独立远程行；账户入口未注册时保留原设置按钮及远程行。更新和连接状态仍在底部独立呈现，空状态行不占空间。账户菜单与回退按钮都打开同一个设置面板；菜单打开的设置或远程弹窗关闭后，焦点返回账户按钮。远程菜单项沿用现有 16px 手机线框图标和菜单原语，打开原有配对弹窗，不另造一层设置页。桌面深链仍保留不可见的设置触发点，以便桌宠和快捷键直接打开指定分区，不增加可见入口。
...
标题栏 48px 窗口拖拽带只承接控件以外的空白；会话头部的层级导航、Agent 操作、打开目录按钮、前后插件入口在 Windows 和 macOS 都必须从拖拽区扣出可点击命中区。当前会话标题为不可重复导航的只读面包屑，Agent 预设名称为只读状态标签；祖先会话标题、Agent Team 和其它实际按钮必须响应点击。
