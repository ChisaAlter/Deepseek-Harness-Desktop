...
- Register every editable `ui-conversation` preference as a volatile Host `Config` field using the shared field schemas. Otherwise `ConfigForm` appears writable while `settings.mutate` rejects omitted fields and reverts the switch. Keep the resident and Session headers on one grid and one padding layer so the new nested slot does not squeeze the title or lower the right column.
- Register all Desktop appearance fields in `ui-theme` as volatile Host configuration so visible wallpaper, glass, font, and related controls can save. Keep the archived list in its own collapsed sidebar section, controlled by the existing `showArchivedList` field under `dsh.workspace.view.v5`; view options must not bypass that setting.
- Render the new `settings.launcher` account entry in the Settings shell and recognize DSHD's actual `window.shell` bridge in the account plugin. Write the vision picker to the new live `llm-vision-fallback` entry and migrate both fresh and previously renamed `settings.yaml.imported` documents once, preserving existing vision routes after upgrade.
- When the official account menu already offers Settings, show only the account launcher at the bottom of the sidebar; show the original Settings button when no account launcher is registered. Update and connection status remain separate, and both paths use the same Settings shell. This keeps both assemblies usable without two identically named entries after sign-in.
- Replay `ui-theme` and `ui-conversation` fields omitted during the merged build's first import from `settings.yaml.imported`, one field at a time. Only write fields declared by the current form schema and lacking an explicit profile override, then record a marker for each section. Do not replay theme preference, font size, or busy-enter, which the merged build already allowed users to edit.
...
- Keeping the account menu and standalone Settings button side by side avoids an entry change, but produces two identically named actions in the signed-in sidebar and leaves close-focus ownership unclear; rejected.
