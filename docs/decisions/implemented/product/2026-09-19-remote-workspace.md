...
以下验收面已全部落地并验证（自动化门禁 + `scripts/verify-remote-workspace-live.cjs` 真机 16/16，详见 feature 卡 `last verified`）：
...
- 全部新 UI 过 `verify-client-ui-i18n` 与 fork 门禁；`check-skip-compose-contract.js` 增 remote 行；`node --test` 覆盖 ensure/overlay/strip/toggle/forensics。
...
- 凭据明文落 `$DSH_HOME/remote-workspaces`（沿用上游行为）：从第三方风险变为第一方安全面；设置 UI 明示保存位置，Windows DPAPI 加密记为 Deferred。
- fork 维护成本：上游迭代活跃，`sync:harness` 不覆盖 `vendor/dsh-remote`（独立插件轨道），后续修复靠手工 port。
- 手测残留：远程 tab UI 选目录、`rw_*` 工具链路、侧栏编辑、开关翻转的界面走查——API 级已由 live 验收脚本覆盖。
