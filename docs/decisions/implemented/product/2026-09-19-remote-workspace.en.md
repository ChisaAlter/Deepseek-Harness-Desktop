...
The feature is implemented, and automated regression coverage exercises mounting, client wiring, host-key checks, binding, sync, forwarding, and the live-runner contract. Those tests use synthetic directories and fake SSH/SFTP clients. The real-machine SSH route flow exists in `scripts/verify-remote-workspace-live.cjs`, but this revision had no isolated SSH fixture configured and did not run real SSH round-trips, live forwarding, or packaged UI acceptance. Those production checks are **NOT RUN (not PASS)**; passing automated gates is not a production acceptance pass. See [TC-RW-*](../../../qa/production-acceptance-test-cases.md) for the production cases.

Implemented user-visible behavior (this list describes product behavior; it does not claim production acceptance):
...
- The changed remote-workspace UI paths are clean under `verify-client-ui-i18n`; the repository-wide scan still reports 28 pre-existing hard-coded strings in unrelated paths, with no findings in the changed feature paths. The fork gates pass; `check-skip-compose-contract.js` gains a remote row; `node --test` covers ensure/overlay/strip/toggle/forensics.
...
- Credentials are stored under `$DSH_HOME/remote-workspaces`. Users may select an OS secret store per machine: Windows CurrentUser DPAPI, macOS Keychain, or Linux `secret-tool`. If secret-store writes fail, the plugin falls back to plaintext in `machines.json` and reports `secret-store-failed` so the password is not silently lost. This option therefore cannot guarantee encryption at rest on every machine. Enforcing encryption, migrating existing plaintext, and recovering from a broken keychain remain separate decisions.
...
- Production acceptance remains outstanding for remote-tab picking, the `rw_*` tool chain, sidebar editing, toggle changes, real SSH, and live forwarding. The live runner's coverage does not mean it has connected to a real machine; all these checks remain **NOT RUN** until results and the CI artifact SHA are recorded.
