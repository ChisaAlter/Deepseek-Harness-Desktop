...
Paths resolve against the repository root: `--porcelain` prints repository-relative paths even when invoked from a subdirectory, so the scan first reads `rev-parse --show-toplevel` and authorizes that root on its own. Registering a subdirectory therefore does not authorize the repository above it. A listed path that disappears between `status` and `lstat` is skipped rather than failing.
