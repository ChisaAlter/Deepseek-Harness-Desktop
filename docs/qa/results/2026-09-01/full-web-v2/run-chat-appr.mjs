...
try {
  ...
  await runCase('CHAT-001', async () => {
    ...
    if (!/dshd-qa-ws-2026-08-30/i.test(logs[3] || '')) throw new Error(`轮4目录名缺: ${logs[3].slice(0, 60)}`);
  ...
  await runCase('CMP-021', async () => {
    ...
    await page.click('#send-btn');
    // Watch 90s：either assistant reply, or visible refusal, or stuck.
    const deadline = Date.now() + 90_000;
    let state = null;
    while (Date.now() < deadline) {
      state = await page.evaluate(() => ({
        ...
        banner: document.querySelector('#banner')?.textContent || '',
        last: [...document.querySelectorAll('#log .assistant')].map((n) => n.textContent || '').pop() || '',
        img: Boolean(document.querySelector('#log img')),
        users: document.querySelectorAll('#log .user').length,
      }));
      if (state.banner || (!state.stop && /收到|图片|image|不支持/i.test(state.last))) break;
      await sleep(3000);
    }
    const file = await shot(page, 'cmp-021-attach');
    if (state?.stop) {
      // stuck run → cancel to clean, report defect.
      await page.click('#stop-btn').catch(() => {});
      throw new Error(`图片+文本模型运行 90s 未回（img=${state.img}）→ DEF-ATTACH-TEXTMODEL：SPA 缺少「模型不支持图片」发送前拦截（桌面附录 vision extra 有该拦截）`);
    }
    if (!state?.img && !state?.banner) throw new Error(`无图无 banner（last=${(state?.last || '').slice(0, 40)}）`);
    return { status: 'Pass', note: `附件轮结束：img=${state.img} last=${(state.last || state.banner).slice(0, 40)}`, evidence: [file] };
  });
