...
1. 设置 → 「用量统计」（`usage-stats`）：KPI（含估算费用卡）、半年窗口内按月可选且支持 UTC 起止日期筛选的紧凑月历热力图、按模型柱/环、Top 会话（含费用列）、导出（含费用列）、「设置」弹层（模型多选 + 峰谷开关 + 自定义价）。热力图日期筛选只影响月历汇总；KPI 和排行榜继续展示全部历史，独立每日图保留自己的 7/14/30 天切换。
