...
- 服务器中继禁止 `chisacode.sh` / 上游 `account_id`。AGPL：`AGPL-SHIPPING.md`。内置 `ayase.cn:443` 是 **TLS 传输默认**，不可作 SPA；公网 SPA 由同域 nginx 提供 `https://ayase.cn/dshd/`，中继只占精确 `/ws` 与 `/health` 路由。
