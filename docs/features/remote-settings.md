...
| **id** | `remote-settings` |
| **status** | `active` |
| **last verified** | 2026-08-29 — 审查缺陷弥补（FlipText 复制、cold 第二 boot、prestart copyLink）。报告：[remote-popover-ux.md](../qa/results/2026-08-29/remote-popover-ux.md)。 |
...
- `qa:remote` 必含 `cold.openShowsQr` / `cold.noBareOfferText` / `cold.copyAndRotateControls`；第二 Electron 冷 boot（`DSH_QA_REMOTE=cold`，开窗前禁止 `setRemote`）；QR 只认 `[data-dsh-remote-qr]`。`prestart-ensure` 校验 `copyLink` / `data-dsh-remote-copy-link`。
