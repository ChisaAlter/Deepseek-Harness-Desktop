...
| **status** | `active` |
| **last verified** | 2026-09-01 — 关闭「会话累计费用」时整条峰谷行隐藏（不再只藏 ¥、留下空闲时段）。此前同日：价格面板模型选择改为 `SettingsSelect`。 |

## User paths

1. 设置 → 界面设置：「会话累计费用」开关位于「会话统计」开关正下方（order 72）。打开后显示峰谷状态行：当 `billedUsage` 投影在线时（任何路线、任何模型状态）同一行显示费用段——已定价模型或官方列显示「当前会话费用：¥X.XX」；未知或未定价的模型显示「没有设置当前模型价格」（提醒 + 入口）；悬停费用段展示当前模型的价格（官方列或峰谷价模型为高峰/空闲两行，单一价格模型为「价格」单行；未设置显示「没有设置价格」）。「设置价格」入口保持其旁。关闭开关即隐藏整条峰谷行（含空闲/高峰时段），绝不留下倒计时；投影缺席只藏费用段、绝不显示编造的 ¥0.00。
...
| Manual / QA | 开启会话累计费用后向 DeepSeek 会话发送消息，费用在 1s 内更新；跨北京时间边界前后各发一条消息，核对峰/谷单价；面板改价后数字立即变化；关闭开关后峰谷行（含空闲/高峰时段）整条消失 |
