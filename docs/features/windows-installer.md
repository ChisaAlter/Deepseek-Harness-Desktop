...
| **status** | `active` |
| **last verified** | 2026-09-12 — 品牌外壳（`installer/` + `wrap-setup.mjs` + `build-installer-stub.mjs`）首次提交入库；此前所有已发布版本（含 v0.2.9）均为裸 electron-builder NSIS 向导。修复链：`isExistingInstall` 只信合法绝对路径并先读 `InstallLocation`（损坏卸载记录不再劫持升级目标）；「启动」前校验已装 exe 存在；新增 `DSHD_SETUP_FORCE_FALLBACK` 测试钩子与 `scripts/verify-setup-install.mjs` 真实 Setup 门禁（release.yml 在 smoke 后、上传前对包装产物做 `/S /D` 实装 + 回退孵化 + 卸载清理验证）。本机验证：re-wrap 后 `/S /D=` 实装出完整树；WV2 坏机实测定回退经典向导（env/controller 成功但浏览器进程秒死 → 10s watchdog → 向导可见，佐证回退链路有效）。旧记录：2026-09-06 v0.2.9 发布验证（Setup SHA256 `1eb5bd7c…`，仅覆盖品牌化 NSIS 向导）见 [发布记录](../qa/results/2026-09-06/candidate-583b6fa/RELEASE-STATUS.md)。 |
...
- 外壳健壮性：WebView2 加载失败、控制器创建失败、页面 ready 超时（`TIMER_READY` 10s）、`ProcessFailed`（ready 前且未在安装中才回退，安装中途死掉不双开内层）任一发生都回退经典向导；回退路径不依赖 WebView2。`DSHD_SETUP_FORCE_FALLBACK=1` 强制跳过 WV2 直走经典向导——给 CI/QA 在无健康 runtime 的机器上覆盖同一条用户降级路径的测试钩子，不影响 `/S` 语义。
...
- 发布链产物验收：windows job 的 packaged smoke 门禁（`smoke:packaged` on `dist/win-unpacked`）位于 `npm run dist` 之后、artifact 上传之前，**阻断**发版；步骤内置两次尝试（连续两次失败=真问题），不设 `continue-on-error`。紧随其后是**真实 Setup 门禁** `scripts/verify-setup-install.mjs`（阻断、无重试、30min 上限）：校验 DSHSTUB 尾（防裸包）、`/S /D` 实装出完整树、卸载注册为合法绝对路径、桌面/开始菜单快捷方式、`DSHD_SETUP_FORCE_FALLBACK` 孵化经典向导子进程、`/S` 卸载清理——win-unpacked smoke 够不到的包装/外壳回归由这层拦。`workflow_dispatch` 默认只构建 Windows；macOS 仅在显式 `include_macos=true` 时构建。本版 Windows 发布不上传 DMG。不得把这两个步骤改造成重复 test.yml 的质量门（`npm test` / `test:gui` 仍禁止进 release.yml）。
...
- `scripts/verify-setup-install.mjs` — 真实 Setup 安装门禁
...
| Kind | What |
| --- | --- |
| Automated | `node --test src/main/installer-branding.test.js src/main/ci-isolation.test.js`（随 `npm test`）：nsis 契约、BMP 几何/位深、nsh 宏白名单、release.yml glob 对齐、tail 格式 buildTail/parseTail round-trip、stub `/S` 透传与回退钉子、app.html 官方 token 白名单；手动候选默认 Windows-only；packaged smoke 位于 dist 后、上传前，两次尝试且无 `continue-on-error`；publish.yml 只晋级 main 同 SHA 候选、校验 Setup 版本名 / SHA256 并禁止重建；release CI 实跑 `smoke:packaged`（win-unpacked 实启 + 内嵌 DSH_SMOKE 断言，阻断发版）+ `verify-setup-install.mjs`（真实 Setup 全链路，阻断发版） |
| Manual / QA | `TC-INST-001`（GUI 安装走查）、`TC-INST-009`（`/S` 覆盖升级）、`TC-INST-010`（卸载）、`TC-INST-012/013` in [production-acceptance-test-cases.md](../qa/production-acceptance-test-cases.md)；GUI 双路径验收：WV2 健康机器验品牌 UI 全流程，`DSHD_SETUP_FORCE_FALLBACK=1`（或无 runtime 环境）验经典向导回退——CI 无法保证 WV2 健康，品牌页只能手动验；每次改品牌位图后对 CI windows artifact 目检欢迎/许可/目录/完成/卸载五页——实机执行清单（artifact 下载/SHA256/逐页 checklist/zh_CN）固化在 [TC-INST-RUNBOOK.md](../qa/results/2026-08-25/installer-branding/TC-INST-RUNBOOK.md) |
