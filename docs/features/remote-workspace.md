...
- `vendor/dsh-remote` 砍 `update.js`：插件不做自更新，桌面统一管更新。`lib/client.js` 由 `plugin-src/client/` 生成；源码启动预检按 mtime 检查，`pack` / `dist` / `dist:mac` 强制重建，打包后删除 esbuild 与可选原生构建依赖。
...
- `vendor/dsh-remote/**`（含 `node_modules` git 跟踪例外）、`src/main/dsh-remote-desktop.js` 及测试（受管块 strip 内建于 ensure，无独立 legacy preset 文件）、`scripts/verify-remote-workspace-live.cjs`（env 门控真机验收）、`scripts/prepare-dsh-remote-client.mjs` 及测试、`scripts/prestart-ensure.mjs`（源码启动构建接线）
