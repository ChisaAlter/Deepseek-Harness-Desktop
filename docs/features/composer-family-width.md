...
| Manual / QA | 开 composerResize → 左右拖动输入卡 → 统计行/Dock 卡同宽联动；关闭开关 → 全部回退；切换会话 + 刷新后尺寸保持 |
