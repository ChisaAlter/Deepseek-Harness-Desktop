...
- 提交候选里超过 100 MiB（`100 * 1024 * 1024`）的文件在提交对话框内醒目标注路径与大小；扫描只读元数据（不 `git add`、不写索引），用 `lstat` 不跟随符号链接，忽略项不出现。仅警告不改行为：排除该文件后其警告消失，按钮仍可点，提交语义不变。扫描失败按「无警告」降级、不阻塞提交；对话框关闭即在途结果作废。
