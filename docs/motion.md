...
| 桌面启动页 | 标志 / 文案 `rise`（8px + fade，错开 0 / 80 / 120 / 160ms）；盖章 `pulse` 1.2s；鲸鱼 SVG `spin` 2s 无限加载循环；日志行 `fade`。进出场时长走官方 token；减弱动效时全部停，加载图换静态头像 | [`boot.css`](../src/renderer/boot.css)。仪器风不得扩散，见 [桌面启动页](design-language.md#桌面启动页) |
