...
- **Composer width linkage**: the cost strip caps its width at `--dsh-composer-resized-width` (seat-published while the composer card is drag-resized) with the resting card cap as fallback, so it follows the input card like the session-stats row; header comment in `src/client/styles.ts` marks the rule.
