...
- `lib/client.js`, the `./client` export, `dsh.client` metadata, and client peer dependencies are retained — the desktop ships the web client from this same vendored project. Its source lives in `plugin-src/client/`, uses `ui-primitives` / `--dsw-alias-*` tokens / typed dictionaries (settings section「远程工作区」, picker remote-flow occupant, sidebar remote-files tab), and is rebuilt by `scripts/prepare-dsh-remote-client.mjs` before source launch and package/distribution builds.
...
Contract: [docs/features/remote-workspace.md](../../docs/features/remote-workspace.md) · Decision: [docs/decisions/implemented/product/2026-09-19-remote-workspace.md](../../docs/decisions/implemented/product/2026-09-19-remote-workspace.md)
