...
function parseArgs(): { sourceHome: string; frozenHomeRoot: string | null; scenario: Scenario } {
  let sourceHome =
    process.env.CHISACODE_BENCHMARK_SOURCE_HOME ?? path.join(os.homedir(), ".chisacode");
  let frozenHomeRoot = process.env.CHISACODE_BENCHMARK_FROZEN_HOME_ROOT ?? null;
  let scenario = (process.env.CHISACODE_BENCHMARK_SCENARIO ?? "snapshotOnly") as Scenario;
...
async function freezeHome(sourceHome: string, requestedRoot: string | null): Promise<string> {
  const frozenHomeRoot =
    requestedRoot ?? mkdtempSync(path.join(os.tmpdir(), "chisacode-real-home-"));
  if (process.env.CHISACODE_BENCHMARK_REUSE_FROZEN_HOME === "1") {
    return frozenHomeRoot;
  }
  const frozenHome = path.join(frozenHomeRoot, ".chisacode");
...
async function main(): Promise<void> {
  ...
  const daemon = await createTestChisaCodeDaemon({
    chisacodeHomeRoot: frozenHomeRoot,
    cleanup: false,
  });
