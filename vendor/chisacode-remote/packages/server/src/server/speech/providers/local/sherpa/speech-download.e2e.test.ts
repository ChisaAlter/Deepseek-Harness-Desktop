...
const RUN = process.env.CHISACODE_SPEECH_E2E_DOWNLOAD === "1";
...
downloadTest(
  ...
  async () => {
    ...
    const chisacodeHomeRoot = mkdtempSync(path.join(tmpdir(), "chisacode-speech-download-"));
    const modelsDir = path.join(chisacodeHomeRoot, ".chisacode", "models", "local-speech");
