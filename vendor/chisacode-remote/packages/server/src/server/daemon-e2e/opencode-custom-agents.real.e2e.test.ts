...
describe("daemon E2E (real opencode) - custom agent discovery", () => {
  ...
  test("custom agents defined in opencode.json appear in availableModes", async () => {
    ...
    writeFileSync(
      ...
      JSON.stringify({
        agent: {
          "chisacode-e2e-custom": {
            description: "Custom agent for ChisaCode daemon E2E test",
            mode: "primary",
          },
        },
      }),
    ...
    try {
      ...
      const custom = snapshot.availableModes.find((m) => m.id === "chisacode-e2e-custom");
      expect(custom).toBeDefined();
      expect(custom!.description).toBe("Custom agent for ChisaCode daemon E2E test");
