...
import {
  ...
  type CreateChisaCodeWorktreeDeps,
} from "./chisacode-worktree-service.js";
import { readChisaCodeWorktreeMetadata } from "../utils/worktree-metadata.js";
...
test("creates a worktree and registers it in the source workspace project without git snapshot lookup", async () => {
  ...
  const result = await createChisaCodeWorktree(
    {
      cwd: repoDir,
...
test.skipIf(isPlatform("win32"))(
  ...
  async () => {
    ...
    const first = await createChisaCodeWorktree(
      {
        cwd: repoDir,
    ...
    const second = await createChisaCodeWorktree(
      {
        cwd: repoDir,
        worktreeSlug: "reuse-me",
        runSetup: false,
...
test("renames an eligible unnamed branch-off worktree once on first agent context", async () => {
  ...
  const created = await createChisaCodeWorktree(
    {
      cwd: repoDir,
  ...
  expect(readChisaCodeWorktreeMetadata(created.worktree.worktreePath)).toMatchObject({
    version: 2,
...
test("does not mutate registries or broadcast when core worktree creation fails", async () => {
  const tempDir = mkdtempSync(path.join(tmpdir(), "chisacode-worktree-service-"));
  cleanupPaths.push(tempDir);
  const deps = createDeps();
...
interface TestDeps extends CreateChisaCodeWorktreeDeps {
...
function createWorkspaceGitSnapshot(cwd: string): WorkspaceGitRuntimeSnapshot {
  ...
  return {
    ...
    git: {
      ...
      currentBranch,
      remoteUrl: null,
      isChisaCodeOwnedWorktree: repoRoot !== mainRepoRoot,
      isDirty: false,
      baseRef: "main",
      aheadBehind: null,
      aheadOfOrigin: null,
