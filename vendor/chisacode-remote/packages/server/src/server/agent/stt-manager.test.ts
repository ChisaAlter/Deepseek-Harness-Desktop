...
describe("STTManager", () => {
  ...
  it("uses CHISACODE_VOICE_LANGUAGE over CHISACODE_DICTATION_LANGUAGE", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
  ...
  it("uses CHISACODE_DICTATION_LANGUAGE when CHISACODE_VOICE_LANGUAGE is unset", async () => {
    const fakeStt = await transcribeWithResolvedVoiceLanguage({
      env: {
  ...
  it("uses streaming segmentation for batch transcription and concatenates segment finals", async () => {
    const original = process.env.CHISACODE_STT_BATCH_COMMIT_EVERY_SECONDS;
    process.env.CHISACODE_STT_BATCH_COMMIT_EVERY_SECONDS = "1";
    ...
    try {
      ...
      if (original === undefined) {
        delete process.env.CHISACODE_STT_BATCH_COMMIT_EVERY_SECONDS;
      } else {
        process.env.CHISACODE_STT_BATCH_COMMIT_EVERY_SECONDS = original;
      }
    }
  });
});
