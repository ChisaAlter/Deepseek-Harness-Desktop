...
import {
  markChisaCodeWorktreeFirstAgentBranchAutoNameAttempted,
...
export interface CreateChisaCodeWorktreeInput extends CreateWorktreeCoreInput {
  projectId?: string;
}

export interface CreateChisaCodeWorktreeResult {
...
export type CreateChisaCodeWorktreeFn = (
...
export interface CreateChisaCodeWorktreeDeps extends CreateWorktreeCoreDeps {
  projectRegistry: Pick<ProjectRegistry, "get" | "upsert">;
  workspaceRegistry: Pick<WorkspaceRegistry, "get" | "list" | "upsert">;
  workspaceGitService: WorkspaceGitService;
}

export async function createChisaCodeWorktree(
  input: CreateChisaCodeWorktreeInput,
  deps: CreateChisaCodeWorktreeDeps,
): Promise<CreateChisaCodeWorktreeResult> {
...
export async function attemptFirstAgentBranchAutoName(options: {
  ...
  let metadata: ReturnType<typeof readChisaCodeWorktreeMetadata>;
  try {
    metadata = readChisaCodeWorktreeMetadata(options.cwd);
  } catch {
    return { attempted: false, renamed: false, branchName: null };
  }
  if (
  ...
  if ((await getCurrentBranchImpl(options.cwd)) !== placeholderBranchName) {
    markChisaCodeWorktreeFirstAgentBranchAutoNameAttempted(options.cwd);
...
function maybeMarkFirstAgentBranchAutoNameEligible(options: {
  ...
  writeChisaCodeWorktreeFirstAgentBranchAutoNameMetadata(createdWorktree.worktree.worktreePath, {
...
async function upsertWorkspaceForWorktree(options: {
  ...
  repoRoot: string;
  worktree: WorktreeConfig;
  deps: Pick<CreateChisaCodeWorktreeDeps, "projectRegistry" | "workspaceRegistry">;
