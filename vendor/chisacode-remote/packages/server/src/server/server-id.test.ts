...
function tmpHome(): string {
  return mkdtempSync(path.join(tmpdir(), "chisacode-server-id-"));
}
...
describe("getOrCreateServerId", () => {
  ...
  beforeEach(() => {
    ...
    delete process.env.CHISACODE_SERVER_ID;
    home = tmpHome();
  });
  ...
  it("creates and persists a stable id per CHISACODE_HOME", () => {
  ...
  it("respects and persists CHISACODE_SERVER_ID override", () => {
    process.env.CHISACODE_SERVER_ID = "test-daemon-id";
