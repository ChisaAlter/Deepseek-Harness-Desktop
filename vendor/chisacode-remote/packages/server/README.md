...
CHISACODE_HOME=~/.chisacode        # Runtime state directory (agents/, etc.)
CHISACODE_LISTEN=127.0.0.1:6767  # Listen address (host:port or /path/to/socket)
```

`CHISACODE_HOME` defaults to `~/.chisacode` and isolates runtime artifacts like `agents/`. `CHISACODE_LISTEN` controls the daemon listen address. For blue/green testing you can run a parallel server without touching production state:

```bash
CHISACODE_HOME=~/.chisacode-blue CHISACODE_LISTEN=127.0.0.1:7777 npm run dev
```
