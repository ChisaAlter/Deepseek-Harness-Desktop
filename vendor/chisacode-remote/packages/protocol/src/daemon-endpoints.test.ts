...
describe("relay websocket URL versioning", () => {
  test("defaults relay URLs to v2", () => {
    const url = new URL(
      buildRelayWebSocketUrl({
        endpoint: "relay.chisacode.sh:443",
        useTls: true,
        serverId: "srv_test",
        role: "client",
      }),
...
describe("relay websocket URLs", () => {
  ...
  test("uses wss for non-443 ports when TLS is enabled", () => {
    const url = new URL(
      buildRelayWebSocketUrl({
        endpoint: "relay.chisacode.sh:6767",
        useTls: true,
        serverId: "srv_test",
        role: "client",
      }),
...
describe("shouldUseTlsForDefaultHostedRelay", () => {
  test("returns true for the hosted ChisaCode relay on port 443", () => {
    expect(shouldUseTlsForDefaultHostedRelay("relay.chisacode.sh:443")).toBe(true);
  });
