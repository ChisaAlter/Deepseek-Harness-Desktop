...
describe("chisacode config schema", () => {
  it("parses an empty config without metadata generation", () => {
    const parsed = ChisaCodeConfigSchema.parse({});
  ...
  it("parses old-style worktree and scripts config unchanged", () => {
    ...
    expect(ChisaCodeConfigSchema.parse(config)).toEqual({
      worktree: {
        setup: ["npm install"],
        teardown: ["npm run clean"],
      },
