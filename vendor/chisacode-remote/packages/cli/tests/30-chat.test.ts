...
try {
...

  {
    ...
    const posted = await ctx.chisacode(
      ["chat", "post", "coord-room", "first message for @agent-1"],
      {
        env: { CHISACODE_AGENT_ID: "00000000-0000-4000-8000-000000000111" },
      },
    );
    ...
    const read = await ctx.chisacode(["chat", "read", "coord-room", "--limit", "10"]);
    ...
    const readJson = await ctx.chisacode(["chat", "read", "coord-room", "--limit", "10", "--json"]);
    ...
    // always wakes it up.
    const waitPromise = ctx.chisacode(["chat", "wait", "coord-room", "--timeout", "30s"]);
    const waitSentinel = waitPromise.then(() => "settled" as const);
    let postedSecond = false;
    for (let attempt = 0; attempt < 60; attempt++) {
      const post = await ctx.chisacode(["chat", "post", "coord-room", "second message"]);
...

  {
    ...
    const duplicate = await ctx.chisacode(["chat", "create", "coord-room"]);
...

  {
    ...
    const deleted = await ctx.chisacode(["chat", "delete", "coord-room"]);
