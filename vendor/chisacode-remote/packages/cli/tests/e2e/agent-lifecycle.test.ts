 ...
 * 2. Create agent with `chisacode run "say hello" --provider claude`
 * 3. List agents with `chisacode ls`
 * 4. Wait for agent with `chisacode wait <id>`
 * 5. Inspect agent with `chisacode inspect <id>`
 * 6. Stop agent with `chisacode stop <id>` and verify it remains inspectable
 * 7. Delete agent with `chisacode delete <id>`
...
async function test_agent_ls_empty(): Promise<void> {
  ...
  const result = await ctx.chisacode(["ls", "--json"]);
...
async function test_agent_inspect(agentId: string): Promise<void> {
  ...
  const result = await ctx.chisacode(["inspect", agentId]);
...
async function test_agent_logs(agentId: string): Promise<void> {
  ...
  const result = await ctx.chisacode(["logs", "--tail", "20", agentId]);
...
async function test_agent_stop(agentId: string): Promise<void> {
  ...
  const result = await ctx.chisacode(["stop", agentId]);
  ...
  const inspectResult = await ctx.chisacode(["inspect", agentId]);
