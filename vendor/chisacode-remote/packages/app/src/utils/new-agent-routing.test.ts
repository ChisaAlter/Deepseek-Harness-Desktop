...
describe("resolveNewAgentWorkingDir", () => {
  ...
  it("falls back to repo root when checkout metadata is unavailable", () => {
    expect(resolveNewAgentWorkingDir("/repo/.chisacode/worktrees/feature", null)).toBe("/repo");
  });

  it("supports windows-style chisacode worktree paths without checkout metadata", () => {
    expect(
      resolveNewAgentWorkingDir("C:\\Users\\me\\repo\\.chisacode\\worktrees\\feature", null),
    ).toBe("C:\\Users\\me\\repo");
  });

  it("returns the main repo root for chisacode-owned worktrees", () => {
    const checkout = {
      isChisaCodeOwnedWorktree: true,
      worktreeRoot: "/repo/.chisacode/worktrees/feature",
      mainRepoRoot: "/repo/main",
    } as unknown as CheckoutStatusPayload;

    expect(resolveNewAgentWorkingDir("/repo/.chisacode/worktrees/feature", checkout)).toBe(
      "/repo/main",
    );
  });
});
