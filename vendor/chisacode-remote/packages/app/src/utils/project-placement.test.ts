...
describe("project-placement", () => {
  ...
  it("normalizes chisacode worktree paths into the parent repo key", () => {
    const placement = deriveProjectPlacementFromCwd(
      "/Users/test/repo/.chisacode/worktrees/feature-x",
    );

    expect(placement.projectKey).toBe("/Users/test/repo");
    expect(placement.projectName).toBe("repo");
    expect(placement.checkout.cwd).toBe("/Users/test/repo/.chisacode/worktrees/feature-x");
  });
  ...
  it("prefers an existing placement when present", () => {
    const existing = {
      ...
      checkout: {
        ...
        isChisaCodeOwnedWorktree: false as const,
        mainRepoRoot: null,
      },
    };
