...
export interface ChisaCodeHomeMetadataForkResult {
  sourceHome: string;
  targetHome: string;
  agentFiles: number;
  agentBytes: number;
...
export function resolveChisaCodeHomePath(value: string): string {
  if (value === "~") {
    return homedir();
  }
  if (value.startsWith("~/")) {
...
export async function forkChisaCodeHomeMetadata(input: {
  sourceHome: string;
  targetHome: string;
}): Promise<ChisaCodeHomeMetadataForkResult> {
  const sourceHome = resolveChisaCodeHomePath(input.sourceHome);
  const targetHome = resolveChisaCodeHomePath(input.targetHome);

  if (sourceHome === targetHome) {
    throw new Error("Refusing to fork ChisaCode metadata onto the same CHISACODE_HOME.");
  }
