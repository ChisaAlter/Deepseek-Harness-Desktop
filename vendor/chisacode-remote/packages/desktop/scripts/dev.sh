...
REMOTE_DEBUGGING_PORT="${CHISACODE_ELECTRON_REMOTE_DEBUGGING_PORT:-9223}"
export CHISACODE_ELECTRON_FLAGS="${CHISACODE_ELECTRON_FLAGS:+$CHISACODE_ELECTRON_FLAGS }--remote-debugging-port=$REMOTE_DEBUGGING_PORT"
...
export CHISACODE_CORS_ORIGINS="*"

echo "══════════════════════════════════════════════════════"
echo "  ChisaCode Desktop Dev"
echo "══════════════════════════════════════════════════════"
...
exec "$ROOT_DIR/node_modules/.bin/concurrently" \
  ...
  "cd '$APP_DIR' && CHISACODE_WEB_PLATFORM=electron npx expo start --port $EXPO_PORT" \
