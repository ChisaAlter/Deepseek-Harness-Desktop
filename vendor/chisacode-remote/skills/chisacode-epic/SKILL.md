...
Heavy-ceremony orchestrator. Runs research → plan → implement → deliver as one resumable flow. The plan file at `~/.chisacode/plans/<slug>.md` is the source of truth and survives compaction.
...
This is a ChisaCode skill. Load the **chisacode** skill first — it carries the surface (worktrees, agents, waiting, scheduling, preferences). Every agent you spawn reads it too.

**Do not use your own subagents.** All agents in this skill are ChisaCode agents, spawned through the ChisaCode MCP. Your harness's native subagent stack is not in play here.
...
- **Worktrees only via ChisaCode.** Never run `git worktree add` yourself.
...
Read `~/.chisacode/orchestration-preferences.json`. /epic dispatches across these provider categories:
...
Create a worktree via ChisaCode. Record the returned path and branch — they go into the plan frontmatter.
...
Persist to `~/.chisacode/plans/<slug>.md`:
...
Never push to main directly. Never force-push without explicit permission. Never merge before CI is green. Archive worktrees via ChisaCode, never `git worktree remove`.
...
6. **Merge** when green — ask the user (`AskUserQuestion`: squash / rebase / merge / wait). Read repo convention from recent merged PRs (`gh pr list --state merged -L 5 --json mergeCommit,title`).
   ...
   ```
7. **Archive the worktree** via ChisaCode. Frontmatter: `status: delivered`, `worktree: null`. Append a Notes line.
...
4. Archive the worktree via ChisaCode. Frontmatter: `status: delivered`, `worktree: null`. Ask about push.
