...
Read the **chisacode** skill. Contrast is the point of a committee, so pick across providers deliberately rather than using whatever the default category would resolve to.
...
Create both agents in parallel via ChisaCode with `[Committee] <task>` titles and the same prompt. Wait for both — not just whichever finishes first.
