...
Read the **chisacode** skill for orchestration preferences — worker and verifier providers come from preferences unless the user names them.

Loops are a CLI primitive: `chisacode loop run`. Manage with `chisacode loop ls`, `chisacode loop inspect <id>`, `chisacode loop logs <id>`, `chisacode loop stop <id>`.
...
8. Launch with `chisacode loop run`.
