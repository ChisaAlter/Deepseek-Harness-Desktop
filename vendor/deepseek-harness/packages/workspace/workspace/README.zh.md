...
从任何存在且完整限定的目录创建项目：`C:\` 等文件系统根目录和普通目录都有效。相对路径、`C:work` 等 Windows 盘符相对路径、不存在的路径和文件都会被拒绝，且不会创建项目。再次添加已登记目录时保留项目 ID 和标题，同时刷新 header 并接纳后来导入的未归属会话；原成员顺序不变，新接纳会话按新到旧排列在前。你可以随时重命名项目，并把它移动到列表中的任意位置：
...
当会话不应再出现在分组中时隐藏它：它会从可见列表中消失，但其会话、历史与在项目中的位置都保持不变。当被隐藏的会话应重新出现时恢复它：它会回到其项目下记录的位置；不属于任何项目时则回到 Ungrouped。项目不再需要时移除它：它离开列表，而其文件夹、文件与会话历史绝不受影响——这些会话变成 Ungrouped。再次添加同一目录会创建新登记并重新接纳历史会话。
...
每次启动也使用同一份头部索引，为仍登记的工作区补全遗漏的历史成员。规范化目录匹配且尚未归属的会话按新到旧置于原成员之前，包括已归档会话；原成员顺序、工作区身份及归档集合保持不变。已删除的登记绝不重建。补全通过实体变更路径写入；写入失败阻止启动，下次启动重试剩余成员。
...
- **目录迁移需要用户处理**——重新添加目录只接纳原 cwd 能解析到该目录的会话，不猜测新路径、不改写会话日志。
