...
从任何存在的目录创建项目：给出路径和可选标题，项目即出现在列表中，新到旧排列。不存在的路径或文件而非目录会被拒绝。再次添加已登记目录时保留项目 ID 和标题，同时刷新 header 并接纳后来导入的未归属会话；原成员顺序不变，新接纳会话按新到旧排列在前。你可以随时重命名项目，并把它移动到列表中的任意位置：
