...
The presets you can choose from come from three sources: the presets shipped inside this package under `presets/`, configured roots, and your own presets under `<dshHome>/.agent-presets`. The picker shows each preset's display name and description; a preset whose composition cannot load is listed with the reason rather than hidden, so you can see what to fix or delete. A preset can also declare `hidden: true` in its `preset.yml` to stay off the roster every client reads — the flag is for a plugin-provisioned preset that exists to compose that plugin's own sessions, not to be picked. Resolving and mounting by id are unaffected, and a hidden preset that cannot load still lists with its reason so its directory can be deleted.
...
A copy dereferences symlinks so it is self-contained, re-tightens the tree to owner-only (`0o600` files keeping their owner-execute bit, `0o700` directories), and creates the root on first copy. The copied `preset.yml` is rewritten: the source's description is kept for the author to edit; its name, roster `order`, and `hidden` flag are dropped, so the copy presents itself as its own preset and stays on the pickers it was authored for. Removal refuses presets that ship with the deployment and clears a user default that named the preset just deleted.
