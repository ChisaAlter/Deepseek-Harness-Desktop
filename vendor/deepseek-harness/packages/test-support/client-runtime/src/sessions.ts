...
export class TestSessions implements ISessions {
  ...
  readonly calls: {
    method: 'create' | 'canReuseBlank' | 'setSubagentCatalogOpen' | 'refreshSubagents' | 'refresh' | 'search' | 'fork' | 'delete'
    args: unknown[]
  }[] = []
  ...
  /** Fixtures have no durable history; tests may explicitly stub a reusable result. */
  async canReuseBlank(sessionId: SessionId, signal?: AbortSignal): Promise<boolean> {
    signal?.throwIfAborted()
    this.calls.push({ method: 'canReuseBlank', args: [sessionId, signal] })
    return false
  }
