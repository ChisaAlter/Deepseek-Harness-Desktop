 ...
 * `bundles: 'template'` loads the shipped template's bundle tuple instead of
 * the profile manifest's (a recovery boot ignores a user-edited tuple).
...
export function loadProfileDirectory(
  ...
  const bundles = options.bundles === 'template'
    ? PROFILE_TEMPLATES[basename(dir)]?.bundles ?? manifest.dsh?.profile?.bundles ?? []
