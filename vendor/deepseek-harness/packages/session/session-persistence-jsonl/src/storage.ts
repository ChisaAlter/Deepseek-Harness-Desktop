...
export class JsonlBackendTracker {
  ...
  /** Close every local handle for one session before destructive removal. */
  async closeSession(id: SessionId): Promise<void> {
    ...
    if (errors.length > 0) throw new AggregateError(errors, `${this.name}: failed to close session "${id}" before delete`)
  }
