...
`list(path?)` 返回一个目录层级：按名称排序的子目录及其绝对路径、`hidden` 标志（POSIX 上为点前缀）、`home` 锚点，以及 `crumbs`——从根到目标的祖先链，其中每个 crumb 都是跳转目标，根以完整路径标注。Windows 上该链以合成的盘符选择器（`\\.\dsh-computer`）开头：其自身层级枚举可进入的盘符根目录（每个盘符的 `stat` 与 300 毫秒探测窗口赛跑，停滞的控制器无法冻结选择器），并拒绝 `createDirectory`。不带路径时列举宿主账户的家目录。单次调用至多返回 `maxEntries` 行（配置项，默认 1,000——GitHub 网页端对目录列举采用的同一上限），被截断的层级会报告 `truncated: true`，供客户端提示层级不完整。指向目录的符号链接会被跟随；断链与循环链接被跳过。
...
- **不枚举网上邻居**——Windows 盘符选择器只枚举盘符根；UNC 共享仍凭完全限定路径进入 `list`，而非浏览。
