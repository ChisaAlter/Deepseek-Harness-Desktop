...
The Models settings page stores the designated route in the live `llm-vision-fallback` configuration entry (`provider` + `model`; both absent disables the feature). The legacy `vision-fallback` section of `settings.yaml` is imported into that entry. The session-controller and local subagent admission gates admit image prompts for text-only main models whenever `ctx.visionFallback.configured()` is true, and the agent loop calls `ctx.visionFallback.rewriteMessages()` before dispatching each request: image blocks bound for a model whose `inputModalities` excludes `'image'` are replaced with description text generated once by the designated vision model. The `read_image` tool's route gate ([`@deepseek-ai/dsh-tool-fs`](../../fs/tool-fs)) likewise admits text-only routes while the service is configured, so tool-read images flow through the same substitution.

Each generated description is appended to the session log as a `vision/describe` event before the main request dispatches, so rewritten requests remain reconstructable from the log and later steps reuse logged descriptions instead of re-describing. The request invariant calls `replayMessages()`, which refuses missing descriptions without invoking a model. The generated durable vocabulary includes this event for history reloads.
