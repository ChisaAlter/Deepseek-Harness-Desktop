...
import {
  FlipText, IconChevronDownOutline14, Menu, RiskConfirmation, SHIELD_OUTLINE_PATH, SHIELD_OUTLINE_STROKE,
...
export function PermissionSelect({
  ...
  // The select RPC and the control-stream projection frame travel on
  ...
  useEffect(() => {
    if (pick === null) return
    if (selection?.currentValue === pick) {
      setPick(null)
      return
    }
  ...
  const submit = (id: string): void => {
    ...
    void select(id)
      .catch(() => false)
      .then((ok) => {
        // A rejected admission publishes no preset events, so no frame will
        // land on the pick — revert immediately.
        if (!ok) setPick(null)
      })
  }
      ...
      <Menu
        ...
        anchor={
          ...
          >
            ...
            <FlipText className={css.triggerLabel} text={currentLabel} />
