...
function standardKit(
  ...
  // session-maybe entries stay mounted across the no-session state. Official
  // storeOf takes the scoped binding; when no session is current, fall back
  // to an empty-key binding so surfaces/terminal stores keep the '' bucket.
  const scopedStoreBinding = scope === 'session-maybe' && scopeBinding?.key === undefined
    ? { key: '' } as ScopedStandardSourceBinding
    : scopeBinding?.key === undefined
      ? undefined
