...
会话标题栏的分体按钮高 32px、圆角 18px，与 Session 日志及 Git 胶囊使用相同描边。文档标题栏保留高 24px、圆角 9px 的紧凑规格。两个标题栏都只显示图标，悬停提示显示默认应用名称或文件定位动作。两者都显示默认动作的图标，在菜单的默认应用后标注“（默认）”，仅在自己的操作执行期间禁用，并通过短暂提示报告失败。目录适配器使用已有的跨平台应用列表，将最后一次成功选择保存在 `dsh.open-in-app.choice` 中；原选择不可用时回退到第一个可用应用。
