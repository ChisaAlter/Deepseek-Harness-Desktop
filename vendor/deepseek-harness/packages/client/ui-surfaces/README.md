...
The store keys descriptors by `sessionId` (`bySession`). `open` upserts singleton files/diff/agents, one preview, and one terminal placeholder. `openFile` keeps the files explorer and adds a sibling `file:` tab. `activate` / `close` / `closeOthers` / `closeToRight` / `closeAll` edit that session's list. Titlebar `toggleSurfaces` writes only layout width and does not clear this store. When the pinned Workspace Controller lacks `openPath`, ui-surfaces installs a Host-RPC-backed base method before wrapping it. On desktop, the wrapper takes paths inside the current Session cwd: the root opens the Files explorer; other paths open a `file:` tab; `.html`, `.htm`, `.xhtml`, and `.pdf` additionally await `previewWorkspaceFile` and activate Browser on a successful loopback URL. SVG and other image, media, text, and source files remain in Files. Missing or failed preview IPC leaves Files and does not fall through to the OS opener; non-desktop and out-of-workspace paths use the base Host operation.
