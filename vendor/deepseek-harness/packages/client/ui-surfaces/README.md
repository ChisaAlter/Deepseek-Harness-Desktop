...
The store keys descriptors by `sessionId` (`bySession`). `open` upserts singleton files/diff/agents, one preview, and one terminal placeholder. `openFile` keeps the files explorer and adds a sibling `file:` tab. `activate` / `close` / `closeOthers` / `closeToRight` / `closeAll` edit that session's list. Titlebar `toggleSurfaces` writes only layout width and does not clear this store. Desktop `workspaces.openPath` intercepts into `openFile` when `listDir` exists and the path is inside the session cwd. For `.html`, `.htm`, `.xhtml`, `.svg`, and `.pdf` it then awaits `previewWorkspaceFile` and opens Browser on a successful loopback URL; a missing or failed IPC leaves Files and does not fall through to the OS opener. When the pinned Workspace service ships without `openPath` (true on `dsh-v0.1.2-alpha.2`), `apply` first installs a body via `ensureBaseOpenPath` that calls `remote.session.openWorkspacePath` (the Host OS opener) and then wraps it, so chat produced-file chips, tool rows, terminal links, and the skills directory keep one vocabulary; only paths the intercept declines reach the Host opener.
