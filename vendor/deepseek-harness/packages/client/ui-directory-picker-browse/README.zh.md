...
每个流程条目还会声明一个 `<flow>.remote` 子槽位，让远程工作区插件（例如 SSH 工作区提供方）把远程面板挂进同一个对话框，经标题行的**本机 / 远程**页签条到达。页签条只在槽位被占用时出现；空槽位下本机浏览器就是整个对话框。远程填充方经 `renderSlot` 收到拾取对话(`open`、`active`、`busy`、`onPicked`、`onCancel`、`onError`），并回报一个**本地**路径——它的镜像工作区——因此持有方的采纳路径不需要任何远程感知。面板首次访问后保持挂载，使其机器状态在页签切换与对话框开合之间存活。
...
对话框是 680×500 的 Miller 分栏视图（在较矮或较窄的视口中限制尺寸），经 `ctx.workspaces` 驱动宿主的 `listDirectory` 与 `createDirectory` 原语。两处注册经嵌套的 `ctx.slots.inject()` 调用作为一次事务性效果安装，因为任一声明条目都可能晚些激活或替换其声明；对话框文案注册在本包自己的 locale 命名空间下，让两份字典作为一个单元落地。每处注册在同一次调用中声明自己的 `<flow>.remote` 子槽位——注册表拒绝同一子键被声明两次，所以两个孔是按持有方分开的键——并把占用 `HostObservable` 绑定成 `useRemoteFlow` 选择器钩子，这就是没有远程填充方时页签条隐藏的机制。浏览类失败留在对话框自己的提示区内，因此本填充从不驱动持有方的 `onError` 分支。node 半部是一个空 `apply`，让插件留在宿主名单上。
