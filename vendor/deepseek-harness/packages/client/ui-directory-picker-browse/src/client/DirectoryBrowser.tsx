...
export interface DirectoryBrowserProps {
  ...
  busy: boolean
  /**
   * A remote-workspace flow mounted beside the local browser as a second
   * dialog tab. Absent leaves the tab strip unrendered and the local pane
   * the whole dialog. `render` produces the remote pane for the `active`
   * state it is called with: the occupant stays mounted once first visited
   * (hidden, still fed a render with `active: false`) so its machine and
   * connection state survives tab switches and dialog closes.
   */
  remote?: {
    /** Render the remote pane; `active` is false while the local tab shows or the dialog is closed. */
 ...
 */
export function DirectoryBrowser({ open, listDirectory, createDirectory, onOpen, onClose, busy, remote, t }: DirectoryBrowserProps) {
  ...
  const activeTab = remote !== undefined && tab === 'remote' ? 'remote' : 'local'
  ...
  const remoteNode = remote !== undefined && remoteVisited
    ? remote.render(open && activeTab === 'remote')
    : null
