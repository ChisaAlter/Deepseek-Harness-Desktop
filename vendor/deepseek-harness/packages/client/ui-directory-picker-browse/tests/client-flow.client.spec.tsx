...
function StatefulRemotePane({ share }: { share: RemoteFlowOwnerProps }) {
  const [machine, setMachine] = useState('machine-before-switch')
  return (
    <div data-testid="remote-pane">
      {`open:${String(share.open)} active:${String(share.active)} busy:${String(share.busy)}`}
      <input
        aria-label="remote machine state"
        value={machine}
...
describe('directory-picker-browse client half', () => {
  ...
  it('shows the local/remote strip while occupied and feeds the occupant the owner share', async () => {
    ...
    const slots = slotProps<typeof BrowseDirectoryFlowHero>({
      occupied: true,
      renderSlot: share => <StatefulRemotePane share={share} />,
    })
    ...
    fireEvent.change(screen.getByRole('textbox', { name: 'remote machine state' }), {
      target: { value: 'machine-after-switch' },
    })
    ...
    fireEvent.click(remoteTab)
    expect((screen.getByRole('textbox', { name: 'remote machine state' }) as HTMLInputElement).value)
      .toBe('machine-after-switch')
  })
