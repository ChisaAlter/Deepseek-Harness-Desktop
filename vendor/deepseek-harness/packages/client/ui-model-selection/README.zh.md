...
与 `ui-conversation` 及命令包一起挂载本插件；composer 随即在待处理指示器旁显示模型位，`/model` 则以弹窗打开同一份目录。composer 在挂载和会话绑定变化时自动加载目录，无需打开菜单，也不提交模型选择。两个表面使用持久化的会话选择与共享宿主目录；首次同步未完成时触发器显示加载文案，目录缺少显示名时回退为 provider/model 标识。
