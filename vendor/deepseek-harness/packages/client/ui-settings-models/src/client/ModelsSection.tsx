 ...
 * (the catalog selector over the provider editor) and declaring a custom model
...
import { Button, IconPlusOutlineRegular, Modal, SegmentedControl, SettingsSelect } from '@deepseek-ai/dsh-client-ui-primitives'
                    ...
                    <div className={styles['field']}>
                      ...
                      <SettingsSelect
                        ...
                        onChange={(id) => {
                          ...
                          /* v8 ignore next -- the selector only lists addable rows */
                          if (picked === undefined) return
                          setEditing(targetOf(picked.row))
                        }}
                        options={addable.map(({ row }) => ({ id: row.entry.provider, label: row.entry.displayName }))}
                      />
                    </div>
                    <ProviderEditor
