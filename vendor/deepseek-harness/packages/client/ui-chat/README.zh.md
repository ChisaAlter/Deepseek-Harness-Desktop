...
文件提及、产物文件和工具行路径会相对 Session cwd 解析后调用共享的 `workspaces.openPath`，使桌面组装可将它们送进 Files 与 Browser，而不是绕过 surfaces 直达 Host 打开器。文件引用提供方同时接收当前查看的会话 ID 与收尾轮次的属主信息，因此继承历史中的链接可以指向 fork 自身。
