...
export class ChatSnapshotBuilder implements ConversationViewBuilder<ChatConversationViewNode, ChatSnapshot> {
  ...
  replace(input: {
    ...
    const nodes = this.editedTurns.replace(this.skillNames.replace(this.referenceLabels.replace(input.nodes)))
  ...
  apply(input: {
    ...
    const referenced = this.referenceLabels.apply(input.upserts, this.store)
    const skilled = this.skillNames.apply(referenced, this.store)
    const upserts = this.editedTurns.apply(skilled, this.store)
