...
File mentions, produced files, and tool-row paths resolve against the Session cwd and call the shared `workspaces.openPath`, so the assembled desktop can route them through Files and Browser instead of bypassing surfaces through the Host opener. File-mention providers receive the viewed Session ID with the closing-turn owner, so links into inherited history can address the fork itself.
