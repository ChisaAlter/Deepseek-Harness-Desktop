...
function expectSeated(c: TestClient): void {
  for (const [name, component] of SEATS) {
    if (name === 'settings.section') {
      // The desktop chrome adds the Interface and About sections beside General.
      expect(ownEntries(c, name).map(entry => entry.component))
        .toEqual([GeneralSection, InterfaceSection, AboutSection])
    } else {
...
describe('ui-settings-general apply', () => {
  ...
  it('fills the seats of the shell it declares, with locale-following section labels', async ({ mock, start }) => {
    ...
    const navigation = c.ctx.get('settingsNavigation') as SettingsNavigation | undefined
    ...
    const iface = ownEntries(c, 'settings.section').find(row => row.component === InterfaceSection)!
    expect(iface.options).toMatchObject({ id: 'interface', order: 6 })
    expect(resolveSlotLabel(iface.options.label)).toBe('界面设置')
    const about = ownEntries(c, 'settings.section').find(row => row.component === AboutSection)!
  ...
  it('the nav label thunk follows the active locale without re-registration', async ({ mock, start }) => {
    ...
    SEATS.forEach(([name], i) => {
      ...
      if (name === 'settings.section') {
        expect(ownEntries(c, name)).toHaveLength(3)
      } else {
  ...
  it('registers the desktop-only harness-restart row only for a shell with both config directions', async ({ mock, start }) => {
    ...
    // gets its own client). The no-bridge case is covered by the main seat test.
    const getConfig = vi.fn()
    const shellRef = window as Window & { shell?: unknown }
    ...
    try {
      ...
      const desktop = await TestClient.start({ roster: webApp }, mock)
      try {
        const entry = ownEntries(desktop, 'settings.general.item')
          .find(item => item.options.id === 'harness-restart')!
  ...
  it('removes every seat and the item declarations when its row unloads', async ({ mock, start }) => {
    const { c } = await client(mock, start)
    expect(c.ctx.slots.spec('settings.general.item')).toBeDefined()
    expect(c.ctx.slots.spec('settings.interface.item')).toBeDefined()
    await c.unload(SELF)
    await c.flush()
