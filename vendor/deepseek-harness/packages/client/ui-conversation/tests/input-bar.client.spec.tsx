...
function bench(over?: BenchOptions) {
  ...
  const props: InputBarProps = {
    ...
    useComposerBeamStyle: bindSnapshotSelector(createSnapshotStore(over?.composerBeamStyle ?? DEFAULT_COMPOSER_BEAM_STYLE)),
...
describe('image draft rail', () => {
  ...
  it('announces a dropped folder as rejected and still drafts the real files', () => {
    ...
    act(() => { attachmentOwner(result.slotCalls).onAddFiles([file], new Set([folder])) })
  ...
  it('announces a folder-only intake without drafting anything', () => {
    ...
    const result = bench({ addFiles })
    act(() => {
      attachmentOwner(result.slotCalls).onAddFiles([], new Set([new File([], 'a'), new File([], 'b')]))
    })
    ...
    act(() => {
      attachmentOwner(result.slotCalls).onAddFiles([], new Set([new File([], '')]))
    })
...
describe('command launcher chrome and control seats', () => {
  ...
  it('thinking beam stays in a pointer-inert overlay that does not wrap toolbar chips', () => {
    ...
    const { view } = bench({
      running: true,
      composerBeam: true,
      permissionEntry: <button type="button" aria-label="访问模式" />,
      composerBeamStyle: {
    ...
    expect(beam?.getAttribute('data-beam-direction')).toBe('counterclockwise')
    expect((beam as HTMLElement).style.getPropertyValue('--dsh-composer-beam-period')).toBe('3.25s')
