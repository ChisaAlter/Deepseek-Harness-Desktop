...
function childEntry(id: typeof CHILD, label: string | undefined): CatalogEntry {
  return label === undefined
    ? { id, createdAt: 0, mode: 'one-shot' }
    : { id, createdAt: 0, mode: 'continuable', label }
}
...
function withCatalog(
  ...
  return {
    ...list,
    projectionsBySession: { ...list.projectionsBySession, [parent]: { values: { subagentCatalog: [...entries] }, state: 'ready', error: null } },
  }
}
