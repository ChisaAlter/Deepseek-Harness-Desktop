...
export interface IConversation {
   ...
   * @param attachmentIds - draft-local attachments added during editing.
...
export class ConversationController extends Service implements IConversation {
  ...
  async edit(messageSeq: number, text: string, attachmentIds: readonly DraftAttachmentId[], signal: AbortSignal): Promise<SubmitOutcome> {
    const session = this.scopedSession('edit')
    if (session.getSnapshot().subagent !== null) return { kind: 'error' }
    return this.sendSession(session, text, attachmentIds, 'queue', signal, messageSeq)
  }
