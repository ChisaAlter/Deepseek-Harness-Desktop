...
export class SessionInputShell implements SessionInput {
  ...
  private edit: {
    ...
    readonly stash: { readonly draft: string; readonly attachmentIds: readonly DraftAttachmentId[] }
  } | undefined
   ...
   * attachments, seed the editor, and redirect submit to the spec's sink.
   */
  beginEdit(spec: InputEditSpec): boolean {
    ...
    this.edit = { spec, stash: { draft: this.snapshot.draft, attachmentIds: this.attachmentIds } }
    this.attachmentIds = []
  ...
  private finishEdit(): void {
    ...
    this.edit = undefined
    this.attachmentIds = edit.stash.attachmentIds
  ...
  submit(mode: InputSubmitMode = 'queue'): void {
    if (this.snapshot.draft.trim() === '' && this.attachmentIds.length > 0) {
      if (this.snapshot.phase === 'plain') {
        ...
        const edit = this.edit
        const sink = edit !== undefined
          ? (text: string, ids: readonly DraftAttachmentId[], signal: AbortSignal) =>
            edit.spec.submit(text, ids, signal)
          : (text: string, ids: readonly DraftAttachmentId[], signal: AbortSignal) =>
            this.deps.defaultSink(text, ids, mode, signal)
        void sink('', attachmentIds, controller.signal).then((outcome) => {
  ...
  private sinkSerialized(
    ...
    const runSink = (text: string): Promise<SubmitOutcome> => this.edit !== undefined
      ? this.edit.spec.submit(text, attachmentIds, attempt.signal)
