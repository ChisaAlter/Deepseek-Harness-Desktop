...
import type {
  SessionListState, SubagentCatalogSnapshot,
...
export function subagentCostSources(
  ...
  while (frontier.length > 0) {
    ...
    for (const parent of frontier) {
      const catalog = list.subagentsByParent[parent]
...
function catalogLabel(
  ...
  const entry = catalog?.entries.find(candidate => candidate.kind === 'child' && candidate.id === childId)
