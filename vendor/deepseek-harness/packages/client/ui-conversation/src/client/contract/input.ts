...
export interface InputEditSpec extends InputEditState {
   ...
   * @param attachmentIds - ordered browser-owned draft attachment ids captured at submit.
   * @param signal - the submit attempt's abort signal.
   */
  submit(text: string, attachmentIds: readonly DraftAttachmentId[], signal: AbortSignal): Promise<SubmitOutcome>
}
...
export interface SessionInput extends InputTarget {
  ...
  /**
   * Begin one edit session: stash the current draft and attachments, seed the
   ...
   * attachments. No-op without a live edit; refused while an admission
