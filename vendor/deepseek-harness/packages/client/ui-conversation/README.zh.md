...
界面设置保留即时 beam 开关，并增加设置图标弹窗，用于调节方向、单圈周期、整体强度、bloom 强度和色相偏移。弹窗使用共享 `ComposerBeam` 渲染器预览，通过 Host-backed 命名空间保存一份完整 `composerBeamStyle`，默认值恢复历史渲染；这些控件不会改变已钉版的 stroke、blur、裁切壳、mask 或圆角几何。
