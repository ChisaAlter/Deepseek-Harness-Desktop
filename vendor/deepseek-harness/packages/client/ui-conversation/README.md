...
The scoped `conversation.edit` sends revised text and draft images through the same Session's attachment admission with an edit target. It returns an admission outcome to the resident composer edit session without invoking fork or navigation; successful admission restores the stashed draft.
...
The blank-session Hero workspace and agent-preset row follows the input card's effective width. It caps at `--dsh-composer-resized-width` when a saved width is present, otherwise at `--dsh-composer-card-max-width`, and centers inside the wider Hero stack so restoring a narrow card cannot leave those controls against the stack's left edge.

First submission retains the editor and moves its stack from the measured Hero position with the shared slow-motion token. Relative positioning preserves fixed menu and dialog coordinates; reduced motion settles immediately, and navigation cancels the movement. The normal composer's footer reserves the statistics line height through admission and projection updates, including the interval before any figures exist.

The running border beam stays in an unfiltered pointer-inert wrapper behind the card body. A 4px rounded clip shell bounds its bloom inside the 6px composer-stack gap; the stroke and inner light align to the 22px card edge, while an outer filter container blurs a masked source without covering dock content. Following Libraries.dev Rotate, the 2px legacy stroke runs at 0.6 opacity behind a rotating conic intensity window, the inner light shares a same-direction dual-conic window, and the bloom source remains 1.5px. Interface Settings exposes clockwise/counterclockwise/ping-pong direction, a 0.8–60s period, intensity, bloom intensity, hue offset, breathing, hue cycling, visual profiles, eight palettes or 2–6 custom colors, 0.5–4px track width, 0–12px blur, night dimming, easing, up to five presets, and v1 JSON clipboard import/export. The resting rim keeps the capsule continuously defined; the colorful beam itself has a transparent trail and must sweep cleanly through every corner without a duplicate stroke `clip-path`. The mode is not a business-state machine: focus, typing, send, completion, and error lights remain out of scope.

Interface Settings keeps the immediate beam Switch and adds a settings-icon modal for the complete running-beam profile. The modal previews the shared `ComposerBeam` renderer, saves the active `composerBeamStyle` and `composerBeamPresets` together through one Host-backed namespace mutation, and restores the historical legacy rendering by default; its controls never alter the 4px clip shell, 22px corners, 1.5px bloom source, masks, or pointer-inert layering.

Appearance Settings carries the opt-in composer typing-effect row: an immediate Switch plus a settings-icon modal. While enabled, `TypingFxLayer` ghosts each freshly typed character at its rendered glyph position on a pointer-inert overlay — the Lexical-owned text DOM is never wrapped or rewritten — and optionally replaces the native caret with a block or underline treatment. Paste, history, draft seeding, programmatic updates, and in-flight IME composition never echo, while a composition-end commit echoes the whole committed span once (diffed against the composition-start snapshot), and the bounded pool retires each ghost on its own animation end. The modal previews the effect, tunes echo style, caret, blink, speed, and echo/caret/text colors (theme, six built-in schemes, or a custom triplet), keeps up to five presets, and exchanges a versioned `dsh-typing-fx` JSON envelope through the clipboard; `typingFx`, `typingFxStyle`, and `typingFxPresets` persist in this package's settings namespace through one Host-backed mutation, and prefers-reduced-motion stops every animation.
...
The Host entry contributes the `ui:custom-instructions` prompt section and the `custom_instructions` prompt variable: the durable `ui-conversation` settings namespace's `customInstructions` field (≤1500 characters, default empty) is appended to the system prompt assembled before every model step, so a standing user instruction reaches every request in every scope — including subagents — while an empty or whitespace-only value contributes nothing. Edits apply live: the next step's assembly reads the committed value, and the loop's system-prompt projection records the changed text.

#### KV Cache effect

The section is the prompt's last block, so an edit invalidates only the suffix tail; every earlier section keeps its cached prefix.
