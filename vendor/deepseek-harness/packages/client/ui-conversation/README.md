...
The running border beam stays in an unfiltered pointer-inert wrapper behind the card body. A 4px rounded clip shell bounds its bloom inside the 6px composer-stack gap; the stroke and inner light align to the 22px card edge, while an outer filter container blurs a masked source without covering dock content. Following Libraries.dev Rotate, the 2px stroke runs at 0.6 opacity behind a rotating conic intensity window, the inner light shares a same-direction dual-conic window, and the `blur(8px)` bloom runs at 0.36 opacity. The resting rim keeps the capsule continuously defined; the colorful beam itself has a transparent trail and must sweep cleanly through every corner without a duplicate stroke `clip-path`.

Interface Settings keeps the immediate beam Switch and adds a settings-icon modal for direction, rotation period, overall intensity, bloom intensity, and hue offset. The modal previews the shared `ComposerBeam` renderer, saves one complete `composerBeamStyle` value through the Host-backed namespace, and restores the historical rendering by default; its controls never alter the pinned stroke, blur, clip shell, masks, or corner geometry.
