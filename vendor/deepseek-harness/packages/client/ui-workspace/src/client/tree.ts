...
function groupByWorkspace(
  ...
  for (const workspace of workspaces) {
    ...
    for (const id of workspace.sessionIds) {
      ...
      accounted.add(id)
      if (summary.presentation !== undefined) continue
  ...
  const stray = list.ids
    ...
    .filter((s): s is SessionSummary =>
      s !== undefined && isNoDirectorySession(s, accounted, scratchCwd)
...
export function deriveGroups(
  ...
  for (const g of groupByWorkspace(list, workspaces, archived, rowState.archivedFilter, view.ungroupedOrder, scratchCwd)) {
...
export function sessionMemberIds(
  list: SessionListState, workspaces: readonly WorkspaceView[] = [], scratchCwd?: string,
): SessionId[] {
  return visibleSessionIds(list, workspaces, [], 'show', scratchCwd)
}
...
export function visibleSessionIds(
  ...
  return list.ids.filter((id) => {
    ...
    return s !== undefined && listed(s, accounted, scratchCwd)
      && sessionVisible(s, current, archived, archivedFilter)
  })
}
