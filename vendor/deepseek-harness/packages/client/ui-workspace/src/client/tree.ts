 ...
 * with caller-projected Session order inside each account. Only two kinds of
 * Session are listed: members of a registered Workspace, and no-directory
 * tasks living in the Host scratch cwd (they trail under the "No workspace
 * folder" section). Any other unaccounted Session belongs to a Workspace that
 * is not registered right now — deleting a registration hides its Sessions,
 * registering the same directory again brings them back — so it appears
 * nowhere: not grouped, not in the flat list, not in search, not in Archived.
 ...
 * order, with members resolved from sessionIds in caller-projected order.
...
function groupByWorkspace(
  ...
  const stray = list.ids
    ...
    .filter((s): s is SessionSummary =>
      s !== undefined && isNoDirectorySession(s, accounted, scratchCwd) && sessionVisible(s, current, archived))
  if (stray.length > 0) {
    groups.push(buildGroup(
      UNGROUPED_KEY,
      undefined,
 ...
 * @param archivedSessionIds - registry-global archive set.
 * @param scratchCwd - Host scratch cwd; unaccounted Sessions outside it are not listed.
 * @returns known listed visible Session ids in list order, including ordinary forks and only the current blank.
 */
export function visibleSessionIds(
  ...
  return list.ids.filter((id) => {
    ...
    return s !== undefined && listed(s, accounted, scratchCwd) && sessionVisible(s, current, archived)
  })
}
...
export function deriveFlat(
  ...
  return sessionIds.flatMap((id) => {
    const summary = list.byId[id]
    return summary === undefined ? [] : [sessionNode(summary, descendants, statuses)]
  })
}
...
export function deriveArchived(
  ...
  const descendants = indexSubagentDescendants(unownedSummaries(list.byId))
  const accounted = accountedIds(workspaces)
  const archiveStatuses: SessionStatuses = new Map()
  ...
  return rows.map(session => sessionNode(session, descendants, archiveStatuses))
}
