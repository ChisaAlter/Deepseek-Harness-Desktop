...
/** Map known Host deletion refusals into the browser's confirmation copy. */
function sessionDeleteErrorText(reason: unknown, t: WorkspaceBrowserProps['t']): string {
  const code = typeof reason === 'object' && reason !== null
    ? ('rpcError' in reason && typeof reason.rpcError === 'object' && reason.rpcError !== null
      && 'code' in reason.rpcError ? reason.rpcError.code : 'code' in reason ? reason.code : undefined)
    : undefined
...
function ArchivedSessionSection({ nodes, currentId, now, expanded, onToggle, onUnarchive, onDelete, renderSlot, t }: {
  ...
  renderSlot: SessionTreeProps['renderSlot']
  t: WorkspaceBrowserProps['t']
}) {
...
> & {
  ...
  /** Real Workspaces in Host group order with browser-projected Session order. */
  workspaces: readonly WorkspaceView[]
  /** Host scratch cwd from the Workspace baseline; gates the no-directory bucket. */
  scratchCwd: string | undefined
  /** Ordered no-directory members, including any pinned blank. */
  ...
  /** Open the delete confirmation for an archived Session. */
...
function SessionTree({
  ...
  rowState, archivedNodes, archivedExpanded, onToggleArchived, onSessionUnarchive, onLeaveArchivedOnly,
  ...
  // Preserve the last visible session run while a group folds so the shared
  // presence wrapper can play its exit transition without the derivation
  // having to keep folded rows in the semantic tree.
  ...
  const groups = useMemo(
    ...
    [list, workspaces, rowState, statuses, expandedGroups, ungroupedSessionIds, scratchCwd],
  )
  useEffect(() => {
  ...
  const renderGroup = (group: GroupNode, depth: number): ReactNode => {
    ...
    const overflowCount = collapsedSessionRows(group.sessions).hiddenCount
    const visible = collapsedSessionRows(group.sessions, sessionLimits[group.key])
    if (group.expanded) heldRuns.current.set(group.key, visible)
    const run = group.expanded ? visible : heldRuns.current.get(group.key) ?? { rows: [], hiddenCount: 0 }
    ...
    const sessions = run.rows
    for (const node of sessions) rowKeys.push(`session:${node.id}`)
    if (overflowCount > 0) rowKeys.push(`overflow:${group.key}`)
      ...
      >
        {workspaceId === undefined ? (
          <TasksSectionHeader
            ...
            onCreate={() => { setGroupExpanded(group.key, true); connectNoDirectory() }}
          />
        ) : <ProjectRowItem
...
function FlatList({
  ...
  onLeaveArchivedOnly, archivedNodes, archivedExpanded, onToggleArchived, onSessionUnarchive, renderSlot,
