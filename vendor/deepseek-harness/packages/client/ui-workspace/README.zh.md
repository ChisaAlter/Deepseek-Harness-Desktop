...
`ctx.uiWorkspace.openSession(target)` 会同步替换其拥有的 `mainView` reference，并让主区域返回 Conversation，而不等待 `reference.ready`，因此历史加载会显示在已经选中的 Session 视图内。目标可以是已知 Session id，也可以是持久的直接父子 subagent 地址；显式地址不要求预先加载 parent catalog。`openWorkspace(id, beforeOpen?)` 和 `forkSession(id)` 仅在请求未被后续导航替代时打开结果；新会话使用 `openWorkspace`。主选择顺序是 `retain(target)` → 对仍有效的请求运行 `beforeOpen(sessionId)` → 提交选择 → 释放先前的 main reference；准备过程被替代或失败时释放新 retain 的 reference。后续导航或 owner 释放会阻止晚到的 UI 提交，但不取消底层 Session 创建。归档主 Session 会释放其 reference 并清除主选择。选择失败时保留当前全局面板。Session 行读取 `usePanelInfo`，在全局面板活跃时不显示 Session 选中样式；仅把焦点移到搜索框或目录选择器不会离开该面板。

Workspace 与无工作目录 New Session 连接器会先按摘要排除明显不合格项，再调用 `ISessions.canReuseBlank(sessionId, signal?)`，然后重新检查当前成员关系和归档状态。候选项若有任意 `session/title`、parent/seed/inherited/subagent 身份、`turn/start`、`system/message`、`user/message`、`assistant/message`、`tool/result`，或当前/历史任一非 null `session/presentation` 就跳过；孤立的 null presentation 在 Host 确认其它条件通过时可以复用。不合格、消失或已改变的候选项会跳过并创建，真实 Host 读取错误或取消则向上传递且不创建。完整的检查、复核和创建操作按目标合并，成功或失败都会释放对应的 in-flight 条目。显式打开带标题的 Session 时保留其标题 pin；New Session 绝不清除或重写它。取舍依据见[空白 Session 复用笔记](../../../.agents/notes/implemented/bug-fix/2026-09-18-blank-session-reuse.zh.md)。
