...
describe('WorkspaceBrowser', () => {
  ...
  it('hides sessions of an unregistered Workspace everywhere until its directory is added again', () => {
    ...
    expect(screen.queryByText('archived-orphan')).toBeNull()
    ...
    // Registering the directory again lists both rows under the selected filter.
  ...
  it('keeps an already-expanded group when the selection moves within it', async () => {
    ...
    rerender(b, { useSessions: hook(sessionState([summary('a', 2), summary('b', 1)], { main: sid('b') })) })
  ...
  it('shows only the current blank session as the localized New Session, excluded from search', () => {
    ...
    rerender(b, { useSessions: hook(sessionState([currentBlank, staleBlank], { main: staleBlank.id })) })
