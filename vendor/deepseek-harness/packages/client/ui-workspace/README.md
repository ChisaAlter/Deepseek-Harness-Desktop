...
`ctx.uiWorkspace.openSession(target)` synchronously replaces the owned `mainView` reference and returns the main area to Conversation without waiting for `reference.ready`, so history loading renders inside the selected Session view. The target may be a known Session id or a durable direct-parent subagent address; an explicit address does not require a preloaded parent catalog. `openWorkspace(id, beforeOpen?)` and `forkSession(id)` open their result only if no later navigation has superseded the request; New Session uses `openWorkspace`. The main selection sequence is `retain(target)` → `beforeOpen(sessionId)` for the still-current request → selection commit → release of the previous main reference; a superseded or failed preparation releases the newly retained reference. Navigation or owner disposal suppresses the late UI commit, not the underlying Session creation. Archiving the main Session releases its reference and clears the main selection. Selection failure leaves a global panel visible. Session rows read `usePanelInfo` to suppress their selected appearance while a global panel is active; search and directory-picker focus alone do not leave that panel.

Workspace and no-directory New Session connectors first filter obvious summary exclusions, then call `ISessions.canReuseBlank(sessionId, signal?)` before rechecking current membership and archive state. A candidate with any `session/title`, parent/seed/inherited/subagent identity, `turn/start`, `system/message`, `user/message`, `assistant/message`, `tool/result`, or any non-null current or historical `session/presentation` is skipped; an isolated null presentation is eligible when the Host confirms it. Ineligible, missing, or changed candidates are skipped and a new Session is created, while a real Host read error or cancellation propagates and does not create. The complete inspect, recheck, and create operation is coalesced per target, with its in-flight entry released on success or failure. Explicitly opening a titled Session preserves its title pin; New Session never clears or rewrites it. Rationale: [blank Session reuse note](../../../.agents/notes/implemented/bug-fix/2026-09-18-blank-session-reuse.md).
...
- **No Session deletion on live rows** — archived sessions restore from the Archived section and the archived-sessions Settings page ([ui-settings-unarchive-sessions](../ui-settings-unarchive-sessions/README.md)); confirmed Delete session is archived-only. Workspace registration deletion does not delete Sessions.
