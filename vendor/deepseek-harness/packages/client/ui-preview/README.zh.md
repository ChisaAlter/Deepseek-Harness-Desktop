...
右边栏 Browser occupant，挂在 `surfaces.browser`（`single`，`session-maybe`，由 ui-surfaces 声明）。仅桌面预览 http(s) 文档。渲染进程拥有地址栏并上报宿主矩形；Electron 通过 `window.shell.preview*` 把 `BrowserView` 贴在该矩形上。约定：[slot 系统标准](../../../.agents/notes/implemented/architecture/2026-07-22-slot-type-chain-implementation.zh.md)。
