...
declare module '@deepseek-ai/dsh-client-ui-slots' {
  interface SlotMap {
    /**
     * Keyed Tool call view dispatched by wire Tool name; register with
     * `key: '<tool name>'` — a typo never renders, an occupied key is
     * replaced, unclaimed keys use the generic row. The owner supplies the
     * call identity and frozen running or settled node through explicit phase
     * props; preparing blocks have no dispatched arguments —
     * useToolCallArgumentsPartial optionally subscribes to their raw prefix.
     */
    'tool.call.toolview': {
      kind: 'keyed'
      scope: 'session'
...
export interface ToolCallCommonProps {
  ...
  loadImage: MessageImageLoader
  /** Render one durable Tool image group through the attachment presentation plugin; absent only in a bare unit-test owner. */
