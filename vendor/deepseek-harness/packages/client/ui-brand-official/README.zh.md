...
本包在 `official` profile 构建中填充侧栏品牌槽位，让桌面与 Web 都显示透明鲸鱼娘头像、同排「鲸屿 / WHALE ISLE」及下一行「BASED ON DEEPSEEK HARNESS」。「屿」使用品牌蓝；头像不带方形底板。标志槽位不另绘图片，收起态显示面板图标。其他构建 profile 保留外壳的鱼形标志与本地构建标签，会话首屏始终使用动画鱼。本包不保留运行时状态，也不影响模型请求。
