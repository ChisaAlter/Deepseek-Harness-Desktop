...
describe('ui-theme host', () => {
  it('registers, validates, and disposes the durable theme namespace with its fiber', async () => {
    ...
    expect(resolveThemeSettings(ctx.settings.get(ns) as ThemeSettings | undefined)).toEqual({
      ...DEFAULT_THEME_SETTINGS, preference: 'dark', fontSize: 16,
    })
  ...
  it('answers each collection with the current durable preference until disposal', async () => {
    ...
    expect(rowText(rows[1])).toContain('const lightTokens = {"--dsh-sidebar-rail-fill":"transparent"}')
