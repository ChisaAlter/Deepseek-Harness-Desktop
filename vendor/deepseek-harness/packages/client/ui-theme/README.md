...
`src/styles/` holds nine sheets imported in order by ui-theme's dynamic client entry: `base.css`, `motion.css`, `corner-shape.css`, `design-platform.css`, `wallpaper.css`, `scrollbar.css`, `gradient-shadow-text.css`, `metallic-paint.css`, and `shiki.css`. The client bundle compiles and injects them as plugin-owned global styles, so unload and HMR remove them with ui-theme. `scrollbar.css` is the sole consumer of the `--dsw-alias-scrollbar-*` tokens and must follow `design-platform.css`, which declares them.
...
`metallic-paint.css` layers a translucent `linear-gradient` sheen over every hovered native `button`: the image rides the button's own background, so it clips to the control's border-radius without touching position, overflow, or pseudo-elements, and the variant's hover fill stays visible underneath. Its bands come only from `color-mix` on the label aliases — `--dsw-alias-label-primary-foreground` highlights (the on-primary text color, so glyphs keep contrast), `--dsw-alias-label-primary` shades, `--dsw-alias-label-secondary` mid tones — inverting with the theme's light/dark half and tinting with custom themes. `prefers-reduced-motion` stops the `background-position` sweep while keeping the static sheen. The Appearance "Button sheen" switch persists `metallicPaintEnabled` (default on); `applyAppearanceDocumentExtras` mirrors it into the `data-dsh-metallic-paint` root attribute both rules key on, so off leaves only the variant hover fill while the sheet itself stays mounted.
