...
export function Tooltip({ label, shortcutKeys, side = 'right', align = 'center', delayMs = 0, gap = 8, disabled = false, portal = false, maxWidth, children }: { label: TooltipLabel; shortcutKeys?: readonly string[] | undefined; side?: TooltipSide; align?: 'center' | 'end'; delayMs?: number; gap?: number; disabled?: boolean; portal?: boolean; maxWidth?: number; children: ReactElement<AnchorProps> }) {
  ...
  useEffect(() => {
    ...
    const fit = () => {
      ...
      const offset = side === 'right' || side === 'left' ? 0 : align === 'end' ? width : width / 2
      const requestedLeft = side === 'left' ? pos.x - width - 10 : pos.x - offset
      const left = Math.max(edgeMargin, Math.min(requestedLeft, window.innerWidth - edgeMargin - width))
      ...
      el.style.top = `${placement === 'right' || placement === 'left' ? (pos.top + pos.bottom) / 2
  ...
  const show = () => {
    ...
    setPos({
      x: side === 'right' ? r.right + 10 : side === 'left' ? r.left : align === 'end' ? r.right : r.left + r.width / 2,
      top: r.top,
      bottom: r.bottom,
    })
    announce(true)
