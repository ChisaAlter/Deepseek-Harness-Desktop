...
import { Menu, IconDeviceOutline16, IconPaperPlaneOutlineMedium, IconSettingsOutlineMedium, IconUserOutlineMedium } from '@deepseek-ai/dsh-client-ui-primitives'
...
export function AccountMenu({
  wide, openSettings, openOnboarding, useAccount, useTheme, useLauncherActions,
  signOut, contactUs, showLogin, start, cancel, renderSlot, t,
}: AccountMenuProps) {
  ...
  const closeAction = (): void => { setSelectedAction(null); triggerRef.current?.focus() }
  return <div className={css.root}>
    <Menu open={open} side="top" portal autoFocus className={css.anchor}
      anchor={<button ref={triggerRef} type="button" className={css.trigger} data-collapsed={!wide} aria-label={t('menu')}
