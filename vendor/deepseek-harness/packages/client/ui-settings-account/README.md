...
In DSHD, the first waiting state with an authorization URL opens the system browser through the Desktop shell. The URL uses the effective theme, repeated states for the same attempt do not reopen the browser, and launch errors leave the copy-link fallback and Host attempt intact. Native Harness Desktop opens the browser through its own main process.
