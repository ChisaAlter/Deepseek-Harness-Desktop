...
describe('blank reuse', () => {
  it('forwards the identity and optional cancellation and returns the Host bit', async ({ bench }) => {
    const b = bench()
    const directCtx = new Context()
    const svc = new ClientSessions(directCtx, b.mock.remote as never)
    const signal = new AbortController().signal
    b.mock.remote.session.blankReuse.mockResolvedValue(ok({ reusable: true }))

    await expect(svc.canReuseBlank(sid('draft'), signal)).resolves.toBe(true)
    expect(b.mock.remote.session.blankReuse).toHaveBeenCalledExactlyOnceWith({ sessionId: sid('draft') }, signal)
    await directCtx.fiber.dispose()
  })

  it('throws the Host RemoteError unchanged instead of SessionCreateError', async ({ bench }) => {
    const b = bench()
    const directCtx = new Context()
    const svc = new ClientSessions(directCtx, b.mock.remote as never)
    const failure = new RemoteError('gateway/internal', 'blank read failed', { sessionId: sid('draft') })
    b.mock.remote.session.blankReuse.mockResolvedValue(err(failure))

    const result = await svc.canReuseBlank(sid('draft')).catch((error: unknown) => error)
    expect(result).toBe(failure)
    expect(result).not.toBeInstanceOf(SessionCreateError)
    await directCtx.fiber.dispose()
  })
})
