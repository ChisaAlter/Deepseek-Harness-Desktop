...
describe('Session Controller Client apply', () => {
  ...
  it('drops a deleted subagent instead of retaining it as idle durable', async ({ mock, start }) => {
    const { sessions } = await bench(start)
    await emit(mock, 'api-session/added', {
      sessionId: sid('session-1'), updatedAt: 1, running: false, blank: true,
    })
    await emit(mock, 'api-session/added', {
      sessionId: sid('child'), origin: 'subagent', parentSessionId: sid('session-1'),
      running: true, blank: false, updatedAt: 1,
    })
    await vi.waitFor(() => {
      expect(sessions.list.getSnapshot().byId[sid('child')]).toMatchObject({ origin: 'subagent', running: true })
    })

    await emit(mock, 'api-session/deleted', sid('child'))
    await vi.waitFor(() => {
      expect(sessions.list.getSnapshot().byId[sid('child')]).toBeUndefined()
    })
  })
