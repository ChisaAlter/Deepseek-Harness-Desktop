...
import { SESSION_SEARCH_RESULT_LIMIT, type SessionPresentation } from '../../types.ts'
...
export class ClientSessions implements ISessions {
  ...
  /** Resolve the Host's advisory blank-identity check with its RemoteError intact. */
  async canReuseBlank(sessionId: SessionId, signal?: AbortSignal): Promise<boolean> {
    const result = await this.manager.canReuseBlank(sessionId, signal)
    if (!result.ok) throw result.error
    return result.value.reusable
  }
