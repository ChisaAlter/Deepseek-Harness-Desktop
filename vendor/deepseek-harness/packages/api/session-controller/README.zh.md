...
图片提示准入要求模型原生支持图片或已配置识图回退服务，仍须校验附件并解析模型路由。
...
`prompt` 的可选 `editMessageSeq` 指定空闲普通会话的最新开启轮次纯文本用户消息，并要求没有待处理工作。Host 在图片准入后校验目标，在 pre-step 再检查历史，然后记录带 surface replacement 及 Host 所有 `source.edit` 元数据的修订消息。会话身份、工作区、标题及模型选择不变。更早轮次保留；旧提示词、回答和工具结果退出模型上下文，仍需使用的注入指令保留。已存事件保持追加式，工具副作用不回滚；已被压缩移出模型上下文的目标拒绝编辑。
...
New Session 复用由 Host 决定，而不是由列表摘要决定。只读 `session.blankReuse({ sessionId })` RPC 会在不激活 Agent 的情况下检查已挂载或持久化历史；缺失 Session 返回 `{ reusable: false }`，真实读取错误与取消则向上传递。`blank` 表示尚未出现 `turn/start`。历史中出现任意 `session/title`、parent/seed/inherited/subagent 身份、`turn/start`、`system/message`、`user/message`、`assistant/message` 或 `tool/result`，Session 就不符合条件。任一时刻出现非 null 的 `session/presentation` 也会使其不符合条件，即使之后以 null presentation 释放了当前展示；孤立的 null presentation 在其他检查通过时仍可复用。普通模型、权限和 plan 配置仍符合条件。Client 通过 `ISessions.canReuseBlank(sessionId, signal?)` 暴露该检查；它不新增 projection cache 字段或版本，也绝不清除标题 pin 或重写旧日志。取舍依据见[空白 Session 复用笔记](../../../.agents/notes/implemented/bug-fix/2026-09-18-blank-session-reuse.zh.md)。
