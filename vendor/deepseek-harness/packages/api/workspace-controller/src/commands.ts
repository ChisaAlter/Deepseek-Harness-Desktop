...
export class WorkspaceCommands {
  ...
  async archiveSession(request: WorkspaceArchiveSessionRequest): Promise<WorkspaceArchiveValue> {
    this.assertNotDeleting(request.sessionId)
    try {
  ...
  private assertNotDeleting(sessionId: SessionId): void {
    const gate = this.ctx.get('sessionDeleteGate') as { has(id: SessionId): boolean } | undefined
    if (gate === undefined) {
      throw new Error('sessionDeleteGate is not mounted')
    }
    if (gate.has(sessionId)) {
      throw failure(
