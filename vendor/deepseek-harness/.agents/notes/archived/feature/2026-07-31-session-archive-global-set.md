...
- UI: the `delete` menu row (visual-only) becomes `archive` (label "Archive session", non-danger styling, no confirmation dialog — a non-destructive action whose worst misfire is list hiding); filtering is one extra arm in `tree.ts`'s `sessionVisible` predicate, with `deriveGroups`/`deriveFlat` taking an `archived` set parameter so all four surfaces (group loop, stray bucket, search, flat) share one source. A bottom **Archived** section lists archived sessions via `deriveArchived`; row click is inert, and the row menu offers **Unarchive session** (no confirmation, does not open) plus danger **Delete session** with a confirmation dialog; that destroy path is [Archived-session log deletion](2026-08-23-archived-session-delete.md).
...
The sidebar exposes an **Archived** section at the bottom when the archive set is non-empty; archived sessions restore only via the **Unarchive session** menu action (row click does nothing). Archived rows also destroy logs through confirmed **Delete session**; the workspace folder is unchanged ([archived-session delete](2026-08-23-archived-session-delete.md)). Data and accounting slots stay intact through archive and unarchive. The `workspace.list` response shape change is a pre-release direct edit (no compatibility layer). The workspace-management e2e pins the full chain (archive → row disappears → unarchive → row returns → still hidden after reload when re-archived, log still present); domain tests pin idempotence, unknown-id rejection, restart recovery, and the pre-field media default upgrade.
