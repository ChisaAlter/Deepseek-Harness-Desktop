...
标签密度由求解后的会话栏宽度决定，而不是尾簇当前宽度，因此收起标签不会让密度振荡。会话栏 ≥ 720px 为 `full`。低于 720px 为 `cozy`：Session log 只留图标（保留 aria-label），`header.actions` 隐藏。低于 560px 为 `compact`：分支触发器和 Initialize Git 也去掉文字。Commit 与分栏开关在按钮仍绘制时保持文字。详情栏打开或尾簇隐藏时密度为 `full`。AppFrame 写入 `data-titlebar-density` 以及尾簇 owner 的 `density` 字段。拥挤密度从不移除 Git 或分栏开关；界面设置可省略对应控件，Git 状态不可用时 Git 组也会独立隐藏。

标题行还独立查询扣除预留后的自身内容宽度。宽度不超过 520px 时，即便整条会话栏仍选 `full`，也收起 `header.actions`；更宽时动作组可缩小并把溢出限制在自己的盒内。这个局部查询不改变尾簇宽度，因此不会引入下文否决的密度反馈环。AppFrame 把测得的尾簇小数宽度向上取整后再发布预留；四舍五入会让名义 8px 间隔出现小数像素溢出。
...
拥挤密度从不从被挤窄的桌面会话栏移除 Git 或分栏开关。分栏开关只在对应的界面设置开关关闭时离开尾簇；Git 状态不可用时 Git 组也会隐藏，与拥挤密度无关。先收的仍是 preset 标签和 Session log 文字。在 `data-compact-header` 仍于 1024px 以下隐藏整簇时，`compact` 档很少出现（该宽度下侧栏最大也让中间栏 ≥ 604px）；保留这一档是为了以后改 compact-header 时不会重新互叠。

## Testing

`titlebar-density.ts` 钉住两个函数。AppFrame 钉住避让、详情栏打开时的 full 密度、surfaces 把中间栏钉在 640px 时的 cozy，以及 compact-header 时 reserve 为 0。Session log 在 cozy 去掉可见标签。BranchMenu 在 compact 去掉分支名。会话栏页头 CSS 钉住 padding 公式、标题行容器查询与动作组收缩、控件 no-drag，以及仍占第 1 行的 blank caption。AppFrame 尾簇 CSS 钉住 8px 间距、max-content 的 `no-drag` 空洞、`--dshd-wco-controls` inset、唯一的 caption drag 带，以及手机菜单的 `no-drag`。`apps/web/tests/desktop-chrome.e2e.ts` 拒绝 Session log / Git / 分栏开关水平互叠，并在 surfaces 打开时打开 branch menu。`apps/web/tests/titlebar-fit.e2e.ts` 在 surfaces 打开时测量动作组、utilities 与尾簇的真实边界。桌面 `src/main/harness-chrome-inject.test.js` 钉住只拥有窗口控件的注入。源码与打包 Electron smoke 在打开 surfaces 后，按真实坐标点击这些尾簇控件。`scripts/verify-titlebar-fit.mjs` 在源码 Electron 中重复边界检查。
