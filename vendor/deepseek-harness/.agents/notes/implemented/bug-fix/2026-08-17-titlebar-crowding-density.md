...
Label density keys off the solved conversation column width, not the cluster's current width, so shrinking a label cannot oscillate the density. `full` at center ≥ 720px. `cozy` below 720px: Session log is icon-only (aria-label kept), `header.actions` hides. `compact` below 560px: the branch trigger and Initialize Git also drop their text. Commit and the panel toggles stay labeled when those buttons are drawn. Density is `full` whenever details is open or the cluster is hidden. AppFrame writes `data-titlebar-density` and the trailing owner `density` field. Crowding density never removes Git or the panel toggles; Interface Settings may omit either group, and unavailable Git status independently hides the Git group.

The title row separately queries its own content width after the reserve. At 520px or less it hides `header.actions` even if the whole conversation column still selects `full`; the actions shrink and clip within their own box at wider widths. This local query never changes the trailing cluster's width, so it does not create the density feedback loop rejected below. AppFrame rounds measured trailing width upward before publishing the reserve; nearest-integer rounding allowed subpixel overflow across the nominal 8px gap.
...
Crowding density never removes Git or the panel toggles from a squeezed desktop conversation. The panel toggles leave only when their Interface Settings switches are off; Git also leaves when its status is unavailable, independently of crowding density. The preset label and Session log text still go first. Compact density is rare while `data-compact-header` still hides the cluster below 1024px (max sidebar at that width leaves center ≥ 604px); the stage is kept so a later compact-header change does not re-open overlap.

## Testing

`titlebar-density.ts` pins the two functions. AppFrame pins reserve, details-open full density, cozy when surfaces pins center at 640px, and compact-header reserve 0. Session log drops the visible label at cozy. BranchMenu drops the ref name at compact. Conversation header CSS pins the padding formula, the title-row container query and action-group containment, control no-drag, and a blank caption that still occupies row 1. AppFrame trailing CSS pins the 8px cluster gap, a max-content `no-drag` hole, the `--dshd-wco-controls` inset, the single caption drag band, and the phone-menu `no-drag`. `apps/web/tests/desktop-chrome.e2e.ts` rejects horizontally overlapping Session log / Git / panel-toggle boxes and opens the branch menu while surfaces is open. `apps/web/tests/titlebar-fit.e2e.ts` checks the real boundaries of actions, utilities, and trailing controls when surfaces opens. Desktop `src/main/harness-chrome-inject.test.js` pins a window-control-only inject. Source and packaged Electron smoke click those trailing controls at real coordinates after surfaces opens. `scripts/verify-titlebar-fit.mjs` repeats the boundary check in source Electron.
