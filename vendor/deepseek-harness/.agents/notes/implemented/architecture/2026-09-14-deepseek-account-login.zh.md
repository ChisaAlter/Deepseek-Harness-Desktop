...
登录申请保存发起界面的客户端元数据，使浏览器授权跟随桌面或设置页的语言，不依赖平台默认值。
...
Platform 客户端身份属于发起调用的界面，而不属于 Host。每个账号操作都携带调用方的客户端版本、当前语言和当前 UTC 偏移，provider 据此与组合出的原生平台推导该次调用的五个 Platform 客户端请求头。客户端身份请求头覆盖部署配置中的同名请求头，因此一个被多方共享的 Host 报告的是每次请求的发起界面，而不是它上一次见到的调用方；更新策略客户端对已安装的壳沿用同一组请求头语义。PlatformSession 有意不携带调用方身份；内嵌客户端自行组装同样的五个请求头，并仅在配置来源发送。

## 替代方案

把协议放在 Electron 会为 Web 使用方重复实现，并让外壳拥有凭证。把 token 放入 DEEPSEEK_API_KEY 会丢失账号授权与用户 API Key 的区别。在持久化前将浏览器回调视为成功会产生虚假成功。等待远程取消确认会使本地取消依赖网络。在 Host 中记住一份客户端身份，会让共享该 Host 或经转发接入的其他调用方被标错。这些方案均被拒绝。
...
赠金通知使用 Platform 提供的文案和订单标识。账号进入登录态（已登录启动、登录成功）时读取一次，每次进入设置页或从充值视图返回时再读取一次，不进行定时轮询。服务端下发的通知立即绘制，包括在打开的设置面板之下，因此展示从不延后。客户端在通知卡完成一帧绘制后或用户关闭卡片时确认订单。不写入任何浏览器存储，因此之后再次登录会重新展示服务端仍在下发的订单。确认失败会在本次登录会话内按带上限的指数退避重试。Platform 的确认记录阻止其他设备继续获取通知，但接口不预留展示资格，因此确认前仍可能并发展示。仅获取成功不会记录为已展示。
