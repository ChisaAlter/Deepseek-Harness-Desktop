...
export async function launchWebScaffold(options: LaunchOptions = {}): Promise<WebScaffold> {
  ...
  const patches: PatchOptions[] = [
    ...
    // Desktop fork: the shipped composition already pins the in-app browse
    // picker deterministically (the web-app bundle patch carries both browse
    // rows), so the upstream -auto disable+insert pair is removed here — it
    // would duplicate the shipped client browse row and fail the boot sweep.
