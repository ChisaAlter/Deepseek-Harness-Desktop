 ...
 * @param {object | Function} [targetRepository] Resolved gh target or cached Git reader.
 * @param {Function} [readRun] Cached Git reader.
 * @returns {Promise<object>}
 */
async function resolveBranchHeadContext(cwd, refName, targetRepository, readRun = runGit) {
  if (typeof targetRepository === 'function') {
    readRun = targetRepository;
 ...
 */
async function lookupOpenPullRequest(cwd, refName, targetRepository, readRun = runGit) {
  if (typeof targetRepository === 'function') {
    readRun = targetRepository;
    targetRepository = null;
  }
  ...
  const headContext = await resolveBranchHeadContext(root, headRef, target, readRun);
...
async function readPullRequest(cwd, refName, targetRepository, readRun = runGit) {
  const looked = await lookupOpenPullRequest(cwd, refName, targetRepository, readRun);
